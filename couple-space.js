...
// ========== 情侣空间未读红点追踪 ==========
// storageKey 前缀 → iframe 内 FEATURES id 映射
const COUPLE_SPACE_FEATURE_MAP = {
  'coupleDiaries_':     'diary',
  'coupleAlbum_':       'album',
  'coupleAnniv_':       'anniversary',
  'coupleChecklist_':   'bucketlist',
  'coupleMessages_':    'message',
  'coupleMoods_':       'mood',
  'coupleTimeline_':    'timeline',
  'coupleLetters_':     'futuremail',
  'coupleGarden_':      'garden',
  'coupleLocations_':   'location',
  'coupleSleep_':       'sleep',
  'coupleFinance_':     'ledger',
};

function _csFeatureFromStorageKey(storageKey) {
  if (!storageKey) return null;
  for (const prefix in COUPLE_SPACE_FEATURE_MAP) {
    if (storageKey.indexOf(prefix) === 0) return COUPLE_SPACE_FEATURE_MAP[prefix];
  }
  return null;
}

function _csUnreadKey(charId) { return 'coupleSpaceUnread_' + charId; }

function _csGetUnread(charId) {
  try { return JSON.parse(localStorage.getItem(_csUnreadKey(charId))) || {}; }
  catch(e) { return {}; }
}

function _csSaveUnread(charId, data) {
  data.lastUpdate = Date.now();
  localStorage.setItem(_csUnreadKey(charId), JSON.stringify(data));
}

// 把某个 feature 标为有未读
function markCoupleSpaceUnread(charId, storageKey, itemTimestamp) {
  if (!charId) return;
  const feature = _csFeatureFromStorageKey(storageKey);
  if (!feature) return;
  const ts = Number(itemTimestamp) || Date.now();

  const data = _csGetUnread(charId);
  const old = data[feature] || { lastReadAt: 0, unreadCount: 0, latestAt: 0 };
  // 同一时间戳(同一条 item)不重复计数
  if (ts <= old.latestAt) return;
  // 已经读过的内容(ts <= lastReadAt)不算未读(只更新 latestAt)
  if (old.lastReadAt > 0 && ts <= old.lastReadAt) {
    data[feature] = {
  ...
  _csSaveUnread(charId, data);

  _csUpdateDesktopBadge();
  _csNotifyIframeUnread(charId);
}

// 把某个 feature 标为已读
function markCoupleSpaceFeatureRead(charId, feature) {
  if (!charId || !feature) return;
  const data = _csGetUnread(charId);
  if (!data[feature] || data[feature].unreadCount === 0) return;
  data[feature] = {
    lastReadAt: Date.now(),
    unreadCount: 0,
    latestAt: data[feature].latestAt,
  };
  _csSaveUnread(charId, data);
  _csUpdateDesktopBadge();
  _csNotifyIframeUnread(charId);
...
// 给桌面图标更新角标(所有 space 总未读数)
function _csUpdateDesktopBadge() {
  const badge = document.getElementById('couple-space-app-badge');
  if (!badge) return;
  let total = 0;
  let spaces = [];
  try { spaces = JSON.parse(localStorage.getItem(COUPLE_SPACE_STORAGE_KEY)) || []; } catch(e) {}
  spaces.forEach(sp => {
    const data = _csGetUnread(sp.charId);
    Object.keys(data).forEach(k => {
      if (k === 'lastUpdate') return;
      if (data[k] && data[k].unreadCount) total += data[k].unreadCount;
    });
  });
  if (total > 0) {
...
// 通过 postMessage 把当前 char 的未读状态发给 iframe
function _csNotifyIframeUnread(charId) {
  const iframe = document.getElementById('couple-space-iframe');
  if (!iframe || !iframe.contentWindow) return;
  // 只发给打开的 char 对应的 iframe
  try {
    if (localStorage.getItem('coupleSpaceLastId') !== charId) return;
  } catch(e) {}
  const data = _csGetUnread(charId);
  // 简化:只发送 feature → { unreadCount, latestAt, lastReadAt }
  const map = {};
  Object.keys(data).forEach(k => {
    if (k === 'lastUpdate') return;
    if (data[k]) map[k] = { unreadCount: data[k].unreadCount || 0, latestAt: data[k].latestAt || 0, lastReadAt: data[k].lastReadAt || 0 };
  });
  iframe.contentWindow.postMessage({ type: 'coupleSpaceUnreadSync', charId, unreadMap: map }, '*');
}

// 初始化未读角标 + 监听跨页面 storage 事件
function _csInitUnread() {
  _csUpdateDesktopBadge();
  window.addEventListener('storage', function(e) {
    if (e.key && e.key.indexOf('coupleSpaceUnread_') === 0) _csUpdateDesktopBadge();
  });
}
...
function sendOrSaveCoupleSpaceData(charId, msgObj, storageKey, itemToSave) {
  ...
  // 标记未读(无论 iframe 是否打开都算 AI 新写入)
  if (itemToSave) {
    try {
      markCoupleSpaceUnread(charId, storageKey, itemToSave.timestamp || itemToSave.createdAt || Date.now());
    } catch(e) { console.error('markCoupleSpaceUnread error:', e); }
  }
}
...
function confirmCoupleSpace(charId) {
  ...
  // 初始化未读追踪:把 lastReadAt 设为 now(),让历史内容(重新绑定恢复的数据)
  // 以及刚创建瞬间之前的本地缓存数据都算已读,避免一打开就是一堆红点。
  if (!localStorage.getItem(_csUnreadKey(charId))) {
    localStorage.setItem(_csUnreadKey(charId), JSON.stringify({
      lastReadAt: Date.now(),
      unreadCount: 0,
      latestAt: 0,
      lastUpdate: Date.now()
...
window.addEventListener('message', function(e) {
  ...
  // --- 未读红点:iframe 通知父页面把某个 feature 标为已读 ---
  if (e.data && e.data.type === 'coupleSpaceFeatureRead') {
    try { markCoupleSpaceFeatureRead(e.data.charId, e.data.feature); }
    catch(err) { console.error('markCoupleSpaceFeatureRead error:', err); }
  }
...
${commentsText}`;
    ...
    if (isGemini) {
      ...
      response = await fetch(`${proxyUrl.replace(/\/+$/, "")}/chat/completions`, {
        method: 'POST',
...
async function handleCoupleSpaceAlbumAiRequest(data) {
  ...
  try {
    ...
    if (genMode === 'pollinations' && result.imagePrompt) {
    ...
    } else if (genMode === 'openai_compat' && result.imagePrompt) {
      try {
        if (typeof generateOpenAICompatibleImageFromPrompt !== 'function') {
          throw new Error('generateOpenAICompatibleImageFromPrompt 未定义');
        }
        const ocResult = await generateOpenAICompatibleImageFromPrompt(result.imagePrompt);
        if (ocResult && ocResult.imageUrl) {
          if (ocResult.imageUrl.startsWith('data:')) {
            imageData = ocResult.imageUrl;
          } else {
            const imgResp = await fetch(ocResult.imageUrl);
            if (imgResp.ok) {
              const blob = await imgResp.blob();
              imageData = await new Promise(resolve => {
                const reader = new FileReader();
      ...
      } catch(e) { console.error('Album OpenAI compat gen failed:', e); }
    }
...
// 初始化情侣空间未读角标(DOM ready 后跑)
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _csInitUnread);
  } else {
    setTimeout(_csInitUnread, 0);
  }
}
