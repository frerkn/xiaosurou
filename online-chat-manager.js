...
class OnlineChatManager {
    constructor() {
        ...
        this.groupHistoryPullDebounceMs = 2000;
        this.lastGroupHistoryPullAt = {}; // groupId -> timestamp，仅用于真人联机群聊历史补拉防抖
    ...
    normalizeOnlineGroupMessageId(data) {
        ...
        const fallbackContent = String(data.message || '').slice(0, 80);
        return `online_group_fallback_${data.groupId || ''}_${data.fromUserId || ''}_${fallbackTimestamp}_${fallbackContent}`;
    }
    ...
    addOnlineGroupMessage(chatId, message, options = {}) {
        ...
        chat.history.sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
        ...
        if (options.updateLastMessage !== false) {
            ...
            chat.timestamp = normalizedMessage.timestamp || Date.now();
        }
    ...
    onRegisterSuccess() {
        ...
        // WebSocket 重连/注册成功后，优先补拉当前打开的真人联机群聊历史
        this.requestCurrentGroupHistory('register_success');
        this.renderChatList();
    }
    ...
    openChat(chatId) {
        ...
        // 打开真人联机群聊时主动拉取服务端内存历史
        this.requestGroupHistory(chat.id, 'open_chat');
    ...
    setupVisibilityListener() {
            ...
            this._onVisibilityChange = () => {
                ...
                if (this.isConnected) {
                    this.requestCurrentGroupHistory('visibility_visible');
                    return;
                }
    ...
    requestGroupHistory(groupId, reason = 'manual') {
        ...
        console.log('[群聊] 请求历史补拉:', groupId, reason);
        this.send({ type: 'get_group_history', groupId: groupId });
    }
    ...
    onReceiveGroupHistory(data) {
        ...
        historyMessages.forEach(item => {
            ...
            const result = this.addOnlineGroupMessage(groupId, msg, { render: false, renderChatList: false });
            if (result.added) mergedCount++;
        });
        ...
        data.groups.forEach(group => {
            ...
            if (!this.chats[chatId]) {
                this.chats[chatId] = {
                    ...
                    owner: group.owner || '',
                    history: group.history || [{ role: 'system', content: `群聊「${group.name}」已恢复`, timestamp: Date.now() }]
                };
            } else {
                // 更新成员列表
    ...
    onMemberRemoved(data) {
        ...
        // 从成员列表移除
        chat.members = chat.members.filter(m => m.userId !== data.memberUserId);
    ...
    removeMember(groupId, memberUserId) {
        ...
        // 从成员列表移除
        chat.members = chat.members.filter(m => m.userId !== memberUserId);

        // 添加系统消息
        const removedMember = chat.members.find(m => m.userId === memberUserId);
        if (removedMember) {
            this.addOnlineGroupMessage(groupId, {
                role: 'system',
                content: `${removedMember.nickname} 被移除出群聊`,
                timestamp: Date.now()
            }, { render: true, renderChatList: false });
    ...
    saveGroupAiContextSize(groupId) {
        ...
        async triggerAiCharacterResponse() {
            ...
            try {
                ...
                // 解析AI回复，提取文本消息。官网 Minimax 直连格式不稳定，按 baseURL 单独适配。
                const isOfficialMinimax = String(proxyUrl || '').toLowerCase().includes('api.minimaxi.com');
                const replyTexts = isOfficialMinimax
                    ? this.normalizeOfficialMinimaxOutput(rawContent, messagesPayload[messagesPayload.length - 1]?.content || '')
                        .map(item => item.content)
                    : this.parseAiCharacterResponse(rawContent);
    ...
    只输出JSON数组，不要输出其他内容。`;
        ...
        normalizeOfficialMinimaxOutput(rawContent, userMessage = '') {
            console.log('[官网Minimax适配] rawContent', rawContent);

            // Minimax 思维链必须在任何解析/提取前移除；只移除完整闭合的 <think>...</think>。
