...
window.initFeatures = function(state, db) {
    ...
    document.getElementById('add-song-search-btn').addEventListener('click', function () {
  // 【0.0.37】改: 打开网易云 App 的搜索 tab (替代老搜索 5 模式 modal)
  if (window.NeteaseMusicApp && window.NeteaseMusicApp.open) {
    window.NeteaseMusicApp.open({ initialTab: 'search' });
  } else {
    alert('网易云模块加载中, 请稍后再试');
  }
});
  ...
  if (window.NeteaseMusicApp && window.NeteaseMusicApp.open) {
    ...
    // 【0.0.37 修复】#toggle-fullscreen-btn 已删除 (用户要求), 加 null 检查避免抛错中断后续 init
    const fullscreenBtn = document.getElementById('toggle-fullscreen-btn');
    if (fullscreenBtn) fullscreenBtn.addEventListener('click', togglePlayerFullscreen);
    ...
    async function runCPhoneInternalRefresh(columnName, refreshFn) {
      window.runtimeDiag?.log?.('CPHONE_REFRESH_START', {
        column: columnName,
        skipRealChat: true
      });
      window.runtimeDiag?.log?.('CPHONE_REAL_CHAT_AUTO_CONTINUE_DISABLED', {
        column: columnName,
        skipRealChat: true
      });
      ...
      try {
        await refreshFn();
        window.runtimeDiag?.log?.('CPHONE_REFRESH_DONE', {
          column: columnName,
          skipRealChat: true
        });
      } catch (error) {
    ...
    document.getElementById('regenerate-char-qq-btn').addEventListener('click', async () => {
      ...
      showCustomAlert("正在执行...", "正在生成新的模拟聊天记录...");

      try {

        await runCPhoneInternalRefresh('qq', handleGenerateSimulatedQQ);

        console.log("CPhone QQ模拟记录生成已完成，已跳过主聊天推进。");

      } catch (error) {
        console.error("生成CPhone QQ模拟记录时出错:", error);
        await showCustomAlert("操作失败", `生成模拟聊天记录时遇到错误: ${error.message}`);
      }
    });
    ...
    document.getElementById('regenerate-char-album-btn').addEventListener('click', () => runCPhoneInternalRefresh('album', handleGenerateSimulatedAlbum));
    ...
    document.getElementById('regenerate-char-browser-btn').addEventListener('click', () => runCPhoneInternalRefresh('browser', handleGenerateBrowserHistory));

    document.getElementById('regenerate-char-taobao-btn').addEventListener('click', () => runCPhoneInternalRefresh('taobao', handleGenerateTaobaoHistory));
    ...
    document.getElementById('regenerate-char-memo-btn').addEventListener('click', () => runCPhoneInternalRefresh('memo', handleGenerateSimulatedMemos));
    document.getElementById('char-memo-detail-back-btn').addEventListener('click', () => switchToCharScreen('char-memo-screen'));

    document.getElementById('regenerate-char-diary-btn').addEventListener('click', () => runCPhoneInternalRefresh('diary', handleGenerateSimulatedDiaries));
    ...
    document.getElementById('regenerate-char-amap-btn').addEventListener('click', () => runCPhoneInternalRefresh('amap', handleGenerateAmapHistory));
    document.getElementById('regenerate-char-usage-btn').addEventListener('click', () => runCPhoneInternalRefresh('usage', handleGenerateAppUsage));
    document.getElementById('regenerate-char-music-btn').addEventListener('click', () => runCPhoneInternalRefresh('music', handleGenerateSimulatedMusic));
    ...
    // 旧豆瓣设置/NPC头像/自定义小组/管理帖子等 DOM 已删除（2026-07-02 替换为热点日报）
    // 原 event listener 全部移除，避免 init 阶段 null reference throw
...
- **直接输出JSON数组，不要添加\`\`\`json等标记，不要输出任何多余的分析文本。**`;
    ...
    document.getElementById('recall-message-btn').addEventListener('click', async () => {
      if (!activeTruthGameMessageTimestamp && !activeWatchTogetherMessageTimestamp && window.activeMessageTimestamp && typeof window.activeMessageData !== 'undefined') {
        const targetTimestamp = window.activeMessageTimestamp;
        const activeMessage = window.activeMessageData;
        const currentChatId = state.currentChatId || state.activeChatId;
        ...
        console.log('[长按菜单删除/重roll入口] targetTimestamp:', targetTimestamp);
        console.log('[长按菜单删除/重roll入口] activeMessage:', activeMessage);
        console.log('[长按菜单删除/重roll入口] currentChatId:', currentChatId);

        if (!currentChatId || !activeMessage) return;

        if (activeMessage.role === 'assistant') {
          try {
            console.log('[重roll] chatId:', currentChatId);
            console.log('[重roll] 目标assistant消息:', activeMessage);

            const ok = window.messageStore
              ? await window.messageStore.deleteMessage(currentChatId, activeMessage)
              : false;

            if (!ok) throw new Error('messageStore.deleteMessage返回false');

            if (Array.isArray(state.currentMessages)) {
              state.currentMessages = state.currentMessages.filter(m => {
                if (activeMessage.id && m.id === activeMessage.id) return false;
                if (activeMessage.timestamp && Number(m.timestamp) === Number(activeMessage.timestamp)) return false;
                if (activeMessage.createdAt && Number(m.createdAt) === Number(activeMessage.createdAt)) return false;
                return true;
              });
            }

            if (state.messageWindows && state.messageWindows[currentChatId]) {
              state.messageWindows[currentChatId] = {
                ...state.messageWindows[currentChatId],
                messages: (state.messageWindows[currentChatId].messages || []).filter(m => {
                  if (activeMessage.id && m.id === activeMessage.id) return false;
                  if (activeMessage.timestamp && Number(m.timestamp) === Number(activeMessage.timestamp)) return false;
                  if (activeMessage.createdAt && Number(m.createdAt) === Number(activeMessage.createdAt)) return false;
                  return true;
            ...
            if (window.messageStore?.refreshChatMeta) {
              await window.messageStore.refreshChatMeta(currentChatId);
            }
          ...
          await handleRegenerateResponse();
            ...
            if (typeof showToast === 'function') {
              showToast('重roll失败，请查看控制台');
            } else {
              await showCustomAlert('重roll失败', '请查看控制台');
            }
          }
          return;
        }
        ...
        try {
          console.log('[删除] chatId:', currentChatId);
          console.log('[删除] message:', activeMessage);
          console.log('[删除] message.id:', activeMessage?.id);
          ...
          if (typeof showToast === 'function') {
            ...
            await showCustomAlert('删除失败', '请查看控制台');
          }
        }
        return;
      }
...
`;
    ...
    if (regenBiliBtn) {
      regenBiliBtn.addEventListener('click', () => runCPhoneInternalRefresh('bilibili', handleGenerateSimulatedBilibili));
    }
    // 2. 绑定回车键搜索
...
请直接回复，不要加任何前缀或后缀。`;
        ...
        if (imageBase64 && typeof describeImageWithVisionApi === 'function') {
          const vr = await describeImageWithVisionApi(imageBase64);
          if (!vr.ok) {
            throw new Error(vr.error || '识图失败，无法继续你画我猜（请检查识图 API 配置与网络）。');
          }
          messagesForApi = [
            { role: 'user', content: `你看我画了什么？[画的内容：${vr.description.trim()}]` }
          ];
        }
    ...
    // 全部/单天视图切换
    const todoViewAllBtn = document.getElementById('todo-view-all-btn');
    if (todoViewAllBtn) {
      todoViewAllBtn.addEventListener('click', () => {
        const isAll = todoViewAllBtn.classList.contains('active');
        window.setTodoViewMode(isAll ? 'day' : 'all');
      });
    }
    ...
    if (todoManageBtn) todoManageBtn.addEventListener('click', window.openTodoManageModal);
    const todoManageCloseBtn = document.getElementById('todo-manage-close-btn');
    if (todoManageCloseBtn) todoManageCloseBtn.addEventListener('click', window.closeTodoManageModal);
    const manageOptions = document.querySelectorAll('.todo-manage-option');
    manageOptions.forEach(opt => {
      opt.addEventListener('click', async () => {
        if (opt.classList.contains('disabled')) return;
        const kind = opt.dataset.kind;
        const labelMap = { ai: 'AI 添加的待办', overdue: '过期的待办', completed: '已完成的待办' };
        const confirmed = await showCustomConfirm("确认清理", `确定要清空所有${labelMap[kind]}吗？此操作不可撤销。`);
        if (confirmed) await window.executeTodoManage(kind);
      });
    });
    ...
    if (regenRedditBtn) {
      regenRedditBtn.addEventListener('click', () => runCPhoneInternalRefresh('reddit', handleGenerateSimulatedReddit));
    }
