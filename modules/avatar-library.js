...
(function () {
  ...
  async function getAvatarDescriptionFromApi(base64Url) {
    ...
    // 优先使用识图 API，其次副 API，最后主 API；地址/密钥实时引用 API 预设库
    const visionConfig = typeof resolveApiSlotConfig === 'function'
      ? await resolveApiSlotConfig('vision')
      : {
    ...
    const mainConfig = typeof resolveApiSlotConfig === 'function'
      ...
      : state.apiConfig;
    const apiConfig = (visionConfig?.proxyUrl && visionConfig?.apiKey && visionConfig?.model)
      ? visionConfig
      : ((secondaryConfig?.proxyUrl && secondaryConfig?.apiKey && secondaryConfig?.model) ? secondaryConfig : mainConfig);
    const {
      proxyUrl,
      apiKey,
      model
    ...
    if (!proxyUrl || !apiKey || !model) {
      throw new Error("主API、副API和识图API均未配置或配置不完整。");
    }
