...
// ============================================================
  ...
  async function readAllMessagesForSingleChatExport(chatId) {
    const batchSize = 1000;

    if (!chatId) return [];

    // 新版拆表后，完整聊天记录存储在 db.messages；chat.history 只代表当前渲染窗口缓存。
    if (db?.messages) {
    ...
    // 仅作为无 messages 表环境的兜底，不做旧版格式转换。
    return Array.isArray(state?.chats?.[chatId]?.history)
      ? state.chats[chatId].history.slice().sort((a, b) => {
        const at = Number(a?.timestamp) || 0;
        const bt = Number(b?.timestamp) || 0;
        return at - bt;
      })
  ...
  async function exportSingleChat() {
    ...
    try {
      const allMessages = await readAllMessagesForSingleChatExport(chat.id);
      ...
      chatDataCopy.history = allMessages;
      chatDataCopy.messageCount = allMessages.length;
      chatDataCopy.exportedMessageCount = allMessages.length;

      const backupData = {
        type: 'EPhoneSingleChat',
        version: 1,
        timestamp: Date.now(),
        messageSchemaVersion: window.messageStore?.MESSAGE_SCHEMA_VERSION || chat.messageSchemaVersion || 1,
  ...
  async function importSingleChat(file) {
    ...
    try {
      ...
      if (Array.isArray(importedChatData.history)) {
        importedChatData.history = importedChatData.history.map((msg, index) => {
          if (!msg || typeof msg !== 'object') return msg;
          const normalized = { ...msg };
          const timestamp = Number(normalized.timestamp) || Date.now();
          normalized.timestamp = timestamp;
          normalized.chatId = currentChatId;
          normalized.id = `${currentChatId}::${timestamp}::${normalized.role || 'unknown'}::${normalized.type || 'text'}::${index}`;
          return normalized;
        });
      }
