...
`;
    ...
    try {
      ...
      const response = isGemini ? await fetch(geminiConfig.url, geminiConfig.data) : await fetch(`${proxyUrl.replace(/\/+$/, '')}/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  async function endVoiceCall() {
    ...
    if (window.voiceCallBgAudio) {
      console.log('[Audio] 挂断通话，立即停止背景音乐');
    ...
    try {
      ...
      if (voiceCallState.isGroupCall) {
        ...
        messagesArray.forEach((msg, index) => {
          ...
          // === 任务3：淡出背景音乐 ===
          if (window.voiceCallBgAudio) {
            console.log('[Audio] AI开始说话，淡出背景音乐');
            const fadeOutInterval = setInterval(() => {
              if (window.voiceCallBgAudio && window.voiceCallBgAudio.volume > 0.05) {
                window.voiceCallBgAudio.volume -= 0.05;
              } else {
