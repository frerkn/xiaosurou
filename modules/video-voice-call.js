...
(function () {
  ...
  // === DIAG v0.5.0 P2.2: 屏幕可视化诊断 (PWA 没 console, 改成浮窗, 临时) ===
  function escapeDiagHtml(s) {
    return String(s == null ? '' : s).replace(/[&<>"']/g, function (c) {
      return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[c];
    });
  }
  function createLive2DDiagPanel() {
    let panel = null;
    let contentEl = null;
    function ensure() {
      if (panel && document.body && document.body.contains(panel)) return panel;
      panel = document.createElement('div');
      panel.id = '__live2d_diag_panel__';
      panel.style.cssText = [
      ...
      titleEl.textContent = '🔍 Live2D 通话挂载诊断 (临时)';
      titleEl.style.cssText = 'color:#ffeb3b;font-size:12px';
      ...
      closeBtn.onclick = function () { if (panel && panel.parentNode) panel.parentNode.removeChild(panel); };
      header.appendChild(closeBtn);
      contentEl = document.createElement('div');
      contentEl.style.cssText = 'white-space:pre-wrap;word-break:break-all';
    ...
    return {
      log: function (msg) {
        try { console.log('[DIAG-V]', msg); } catch (e) {}
        try {
          ensure();
          const line = document.createElement('div');
          line.textContent = String(msg == null ? '' : msg);
          line.style.cssText = 'margin:0;padding:1px 0;color:#fff';
          contentEl.appendChild(line);
          if (panel && typeof panel.scrollTop === 'number') panel.scrollTop = panel.scrollHeight;
        } catch (e) {}
      },
      warn: function (msg) {
        try { console.warn('[DIAG-V]', msg); } catch (e) {}
        try {
          ensure();
          const line = document.createElement('div');
          line.textContent = String(msg == null ? '' : msg);
          line.style.cssText = 'margin:0;padding:1px 0;color:#ff9800;font-weight:bold';
          contentEl.appendChild(line);
          if (panel && typeof panel.scrollTop === 'number') panel.scrollTop = panel.scrollHeight;
        } catch (e) {}
      },
      err: function (msg) {
        try { console.error('[DIAG-V]', msg); } catch (e) {}
        try {
          ensure();
          const line = document.createElement('div');
          line.textContent = String(msg == null ? '' : msg);
          line.style.cssText = 'margin:0;padding:2px 0;color:#ff5252;font-weight:bold;background:rgba(255,82,82,0.1);border-left:3px solid #ff5252;padding-left:6px';
          contentEl.appendChild(line);
          if (panel && typeof panel.scrollTop === 'number') panel.scrollTop = panel.scrollHeight;
        } catch (e) {}
      },
      ok: function (msg) {
        ...
        try {
          ...
          const line = document.createElement('div');
          line.textContent = '✓ ' + String(msg == null ? '' : msg);
          line.style.cssText = 'margin:0;padding:1px 0;color:#69f0ae';
          contentEl.appendChild(line);
          if (panel && typeof panel.scrollTop === 'number') panel.scrollTop = panel.scrollHeight;
        } catch (e) {}
      },
      close: function () { try { if (panel && panel.parentNode) panel.parentNode.removeChild(panel); } catch (e) {} }
    };
  }
  ...
  async function mountLive2DForCall(chat) {
    ...
    // === DIAG-V v0.5.0 P2.2: 视频通话挂载可视化诊断 (临时, 排查后删) ===
    // 屏幕浮窗, PWA 没 console 也能看. 8 步逐步记录, 不改逻辑.
    const diag = createLive2DDiagPanel();
    diag.log('===== 视频通话 Live2D 挂载诊断 START =====');
    diag.log('UA: ' + (navigator.userAgent || 'n/a'));
    diag.log('chat.id: ' + (chat && chat.id));
    diag.log('chat.name: ' + (chat && chat.name));
    diag.log('canvas 父 screen: ' + (screen && screen.id));
    diag.log('---');
    diag.log('[1/8] activeModelId = ' + JSON.stringify(activeId) + ' (type=' + typeof activeId + ', truthy=' + !!activeId + ')');

    if (activeId) {
      let data = null;
      let idbErr = null;
      try {
        data = await window.Live2DStorage.getModel(activeId);
        diag.ok('[1/8] getModel 返回');
      } catch (e) {
        idbErr = e;
        diag.err('[1/8] getModel THREW: ' + (e && (e.message || e)));
      }
      if (!idbErr && !data) {
        let allModels = [];
        try { allModels = (await window.Live2DStorage.listModels()) || []; } catch (e) {}
        diag.err('[1/8] getModel 返回 null — ' + activeId + ' 不在 IDB');
        diag.log('  IDB 模型: ' + JSON.stringify(allModels.map(function (m) { return { id: m.id, name: m.name }; })));
        diag.log('  localStorage live2d.activeModelId = ' + JSON.stringify(localStorage.getItem('live2d.activeModelId')));
        diag.log('  localStorage live2d.activeModelId.' + chat.id + ' = ' + JSON.stringify(localStorage.getItem('live2d.activeModelId.' + chat.id)));
      } else if (data) {
        diag.log('  data.modelPath = ' + JSON.stringify(data.modelPath));
        diag.log('  data.name = ' + JSON.stringify(data.name));
        diag.log('[2/8] data.files.size = ' + data.files.size);
        let fi = 0;
        for (const k of data.files.keys()) { if (fi < 12) diag.log('  files[' + fi + ']: ' + k); fi++; }
        if (fi > 12) diag.log('  ... 共 ' + fi + ' 个');
        const modelPathBlob = data.files.get(data.modelPath);
        if (modelPathBlob) {
          diag.ok('[2/8] modelPath blob 存在, type=' + JSON.stringify(modelPathBlob.type) + ', size=' + modelPathBlob.size);
        } else {
          diag.err('[2/8] modelPath blob 缺失 (data.modelPath=' + data.modelPath + ')');
        }
        ...
        try {
          ...
          diag.ok('[2/8] model3.json 解析 OK, Version=' + modelJson.Version);
        } catch (e) {
          diag.err('[2/8] model3.json 解析 FAILED: ' + (e && (e.message || e)));
        }

        if (modelJson) {
          const refs = (modelJson.FileReferences || modelJson.fileReferences) || {};
          diag.log('  FileReferences 字段: ' + JSON.stringify(Object.keys(refs)));
          diag.log('  Moc = ' + JSON.stringify(refs.Moc) + ' (IDB has key: ' + (refs.Moc ? data.files.has(refs.Moc) : 'N/A') + ')');
          const texList = Array.isArray(refs.Textures) ? refs.Textures : [];
          diag.log('  Textures count = ' + texList.length);
          texList.forEach(function (t, idx) { if (idx < 4) diag.log('    Textures[' + idx + '] = ' + JSON.stringify(t)); });
          if (texList.length > 4) diag.log('    ... 共 ' + texList.length + ' 张贴图');
          const baseDir = data.modelPath.substring(0, data.modelPath.lastIndexOf('/') + 1);
          diag.log('  baseDir = ' + JSON.stringify(baseDir));

          // ===== 模拟 loader 完整流程 =====
          // 3. URL.createObjectURL 每个 file
          const urlMap = new Map();
          let blobUrlOk = 0, blobUrlFail = 0;
          for (const [path, blob] of data.files.entries()) {
            try {
              ...
              blobUrlFail++;
              diag.err('[3/8] URL.createObjectURL FAILED for ' + path + ': ' + (e && (e.message || e)));
            }
          }
          diag.ok('[3/8] URL.createObjectURL 成功 ' + blobUrlOk + ' / ' + data.files.size + (blobUrlFail > 0 ? ', 失败 ' + blobUrlFail : ''));

          // 4. 改写 model3.json (模拟 loader.js:252-281)
          const resolveBlob = function (rel) {
            if (!rel) return rel;
            if (rel.startsWith('blob:')) return rel;
            const full = baseDir + rel;
            return urlMap.get(full) || rel; // fallback 原相对路径
          };
          const rewritten = JSON.parse(JSON.stringify(modelJson));
          const rrefs = rewritten.FileReferences || rewritten.fileReferences;
          let fallbackCount = 0;
          if (rrefs) {
            if (rrefs.Moc) rrefs.Moc = resolveBlob(rrefs.Moc);
            if (Array.isArray(rrefs.Textures)) rrefs.Textures = rrefs.Textures.map(resolveBlob);
            if (rrefs.Physics) rrefs.Physics = resolveBlob(rrefs.Physics);
            if (rrefs.Pose) rrefs.Pose = resolveBlob(rrefs.Pose);
            if (rrefs.DisplayInfo) rrefs.DisplayInfo = resolveBlob(rrefs.DisplayInfo);
            if (rrefs.Motions && typeof rrefs.Motions === 'object') {
              Object.keys(rrefs.Motions).forEach(function (g) {
                rrefs.Motions[g] = (rrefs.Motions[g] || []).map(function (m) {
                  const f = m && m.File ? resolveBlob(m.File) : undefined;
                  if (m && m.File && f === m.File && !m.File.startsWith('blob:')) fallbackCount++;
                  return Object.assign({}, m, { File: f });
                });
              });
            }
            if (Array.isArray(rrefs.Expressions)) {
              rrefs.Expressions = rrefs.Expressions.map(function (e) {
                const f = e && e.File ? resolveBlob(e.File) : undefined;
                if (e && e.File && f === e.File && !e.File.startsWith('blob:')) fallbackCount++;
                return Object.assign({}, e, { File: f });
              });
            }
            // 检查 Moc / Textures 是不是 fallback
            if (rrefs.Moc && !rrefs.Moc.startsWith('blob:')) fallbackCount++;
            if (Array.isArray(rrefs.Textures)) rrefs.Textures.forEach(function (t) { if (t && !t.startsWith('blob:')) fallbackCount++; });
          }
          diag.log('[4/8] 改写后, fallback 走原相对路径的引用数: ' + fallbackCount + ' (0=全部解析成 blob URL)');
          if (rrefs) {
            diag.log('  改写后 Moc: ' + JSON.stringify(rrefs.Moc));
            if (Array.isArray(rrefs.Textures)) diag.log('  改写后 Textures[0]: ' + JSON.stringify(rrefs.Textures[0]));
          }

          // 5. 把改写后的 model3.json 转成新 blob + blob URL
          let rewrittenUrl = null;
          try {
            const rewrittenBlob = new Blob([JSON.stringify(rewritten)], { type: 'application/json' });
            rewrittenUrl = URL.createObjectURL(rewrittenBlob);
            diag.ok('[5/8] rewritten model3.json blob URL 创建 OK, type=' + rewrittenBlob.type);
            diag.log('  URL: ' + rewrittenUrl.substring(0, 60) + '...');
          } catch (e) {
            diag.err('[5/8] rewritten model3.json blob URL 创建 FAILED: ' + (e && (e.message || e)));
          }

          // 6. fetch(rewritten model3.json) — 模拟 PIXI Loader 抓 model3.json 的一步
          if (rewrittenUrl) {
            try {
              const r = await fetch(rewrittenUrl);
              diag.ok('[6/8] fetch(rewritten model3.json) status=' + r.status + ' type=' + r.type + ' ok=' + r.ok);
              const t = await r.text();
              diag.log('  body length: ' + t.length);
              try { JSON.parse(t); diag.ok('  body JSON.parse OK'); }
              catch (e) { diag.err('  body JSON.parse FAILED: ' + (e && (e.message || e))); }
            } catch (e) {
              diag.err('[6/8] fetch(rewritten model3.json) FAILED: ' + (e && (e.message || e)));
              if (e && e.stack) diag.log('  stack: ' + e.stack.split('\n').slice(0, 3).join(' | '));
            }
          }

          // 7. fetch(Moc) — 模拟 loader 抓 moc3 的一步
          const mocResolved = refs.Moc ? resolveBlob(refs.Moc) : null;
          if (mocResolved && mocResolved.startsWith('blob:')) {
            try {
              const r = await fetch(mocResolved);
              diag.ok('[7/8] fetch(Moc blob) status=' + r.status + ' type=' + r.type + ' ok=' + r.ok);
              const ab = await r.arrayBuffer();
              diag.log('  Moc response size: ' + ab.byteLength + ' bytes');
            } catch (e) {
              diag.err('[7/8] fetch(Moc blob) FAILED: ' + (mocResolved.substring(0, 50)) + ' ' + (e && (e.message || e)));
            }
          } else {
            diag.warn('[7/8] Moc 解析成非 blob URL: ' + JSON.stringify(mocResolved) + ' (loader 之后 PIXI fetch 这个 URL 会失败)');
          }

          // 8. fetch(Texture[0]) — 模拟 loader 抓贴图的一步
          if (refs.Textures && refs.Textures[0]) {
            const texRef = refs.Textures[0];
            const texResolved = resolveBlob(texRef);
            const texFullPath = baseDir + texRef;
            const texBlob = data.files.get(texFullPath);
            diag.log('[8/8] Texture[0] ref: ' + texRef);
            diag.log('  IDB 里的 Blob: ' + (texBlob ? '存在' : '缺失'));
            if (texBlob) {
              diag.log('  Texture[0] Blob.type = ' + JSON.stringify(texBlob.type) + ' (期望 image/png)');
              diag.log('  Texture[0] Blob.size = ' + texBlob.size);
            }
            diag.log('  Texture[0] 解析后: ' + (texResolved.startsWith('blob:') ? 'blob URL ✓' : '非 blob: ' + texResolved));
            if (texResolved.startsWith('blob:')) {
              try {
                const r = await fetch(texResolved);
                diag.ok('[8/8] fetch(Texture[0] blob) status=' + r.status + ' type=' + r.type + ' ok=' + r.ok);
                const ab = await r.arrayBuffer();
                diag.log('  Texture[0] response size: ' + ab.byteLength + ' bytes');
              } catch (e) {
                diag.err('[8/8] fetch(Texture[0] blob) FAILED: ' + (texResolved.substring(0, 50)) + ' ' + (e && (e.message || e)));
              }
            } else {
              diag.err('[8/8] Texture[0] 不是 blob URL, PIXI fetch 会失败: ' + texResolved);
            }
            // 附加: 同样 Blob 走 <img> 解码能不能成功 (跟 fetch 路径对比)
            if (texBlob) {
              const imgUrl = URL.createObjectURL(texBlob);
              const imgResult = await new Promise(function (resolve) {
                const i = new Image();
                i.onload = function () { resolve({ ok: true, w: i.naturalWidth, h: i.naturalHeight }); try { URL.revokeObjectURL(imgUrl); } catch (e) {} };
                i.onerror = function () { resolve({ ok: false }); try { URL.revokeObjectURL(imgUrl); } catch (e) {} };
                setTimeout(function () { if (!i.complete) { resolve({ ok: false, timeout: true }); try { URL.revokeObjectURL(imgUrl); } catch (e) {} } }, 5000);
                i.src = imgUrl;
              });
              if (imgResult.ok) diag.ok('[8/8] <img> 直接解码 (非 fetch 路径): OK ' + imgResult.w + 'x' + imgResult.h);
              else diag.warn('[8/8] <img> 直接解码 FAILED ' + JSON.stringify(imgResult));
            }
          } else {
            diag.warn('[8/8] Textures[0] 缺失, 无法测贴图 fetch');
          }
        }
      }
    }
    diag.log('===== pre-mount 诊断完成 =====');
    // === DIAG-V END (pre-mount) ===
    ...
    diag.log('===== 调 mountLive2DFromIDB =====');
    diag.log('canvas: ' + canvas.id + ', clientW=' + canvas.clientWidth + ', clientH=' + canvas.clientHeight);
    diag.log('canvas.parentElement: ' + (canvas.parentElement && canvas.parentElement.id) + ', parentH=' + (canvas.parentElement && canvas.parentElement.clientHeight));
    ...
    const mountEnd = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
    diag.log('mount 用时: ' + Math.round(mountEnd - mountStart) + 'ms');
    if (mountErr) {
      diag.err('===== mountLive2DFromIDB THREW =====');
      diag.err('error.name: ' + mountErr.name);
      diag.err('error.message: ' + (mountErr.message || String(mountErr)));
      diag.err('error.stack: ' + (mountErr.stack || ''));
      diag.log('（我的 pre-mount 诊断正常跑完, loader 内部某步骤抛错）');
    } else if (result) {
      diag.log('===== mountLive2DFromIDB 返回 =====');
      diag.log('result.success = ' + result.success);
      if (result.error) {
        diag.err('result.error.name: ' + result.error.name);
        diag.err('result.error.message: ' + (result.error.message || String(result.error)));
        diag.err('result.error.stack: ' + (result.error.stack || ''));
      }
      if (result.app) diag.log('result.app 存在');
      if (result.model) diag.log('result.model 存在');
    } else {
      diag.err('mount 返回 undefined / null');
    }

    if (!result || !result.success) {
      console.warn('[Live2D] mount failed:', result && result.error, '— restoring original video area');
