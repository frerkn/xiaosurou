...
(function () {
  ...
  async function findChatMessageByTimestamp(chat, timestamp, options = {}) {
    if (!chat || timestamp == null) return null;

    const normalizedTimestamp = Number(timestamp);
    const includeHidden = options.includeHidden !== false;
    const matchesTimestamp = message => Number(message?.timestamp) === normalizedTimestamp;
    const isAllowed = message => includeHidden || !message?.isHidden;
    const findInList = list => Array.isArray(list) ? list.find(message => matchesTimestamp(message) && isAllowed(message)) : null;

    let message = findInList(chat.history);
    if (message) return message;

    message = findInList(state.currentMessages);
    if (message) return message;

    if (window.messageStore?.getWindowMessages) {
      message = findInList(window.messageStore.getWindowMessages(chat.id));
      if (message) return message;
    }

    if (window.messageStore?.getMessageByTimestamp) {
      message = await window.messageStore.getMessageByTimestamp(chat.id, normalizedTimestamp);
      if (message && isAllowed(message)) return message;
    }
  ...
  async function copyMessageContent() {
    if (!activeMessageTimestamp) return;
    const chat = state.chats[state.activeChatId];
    const message = await findChatMessageByTimestamp(chat, activeMessageTimestamp);
    if (!message) return;
  ...
  async function saveEditedMessage(timestamp, simpleContent = null) {
    ...
    const normalizedTimestamp = Number(timestamp);
    let originalMessage = chat.history.find(m => Number(m.timestamp) === normalizedTimestamp);

    if (!originalMessage && window.messageStore?.getMessageByTimestamp) {
      originalMessage = await window.messageStore.getMessageByTimestamp(chat.id, normalizedTimestamp);
    }

    if (!originalMessage) {
      await showCustomAlert('保存失败', '找不到要编辑的原消息，已停止保存。');
      return;
    }
    ...
    const rawContent = blocks.find(content => content && content.trim());
    if (!rawContent) {
    ...
    switch (parsedResult.type) {
      case 'text':
        updatedFields.content = parsedResult.content;
        break;
      case 'offline_text':
        if (parsedResult.content) {
          updatedFields.content = parsedResult.content;
          delete originalMessage.dialogue;
          delete originalMessage.description;
        } else {
          updatedFields.dialogue = parsedResult.dialogue;
          updatedFields.description = parsedResult.description;
          delete originalMessage.content;
        }
        break;
      case 'quote_reply': {
        updatedFields.content = parsedResult.reply_content;
        const originalQuotedMsg = chat.history.find(m => Number(m.timestamp) === Number(parsedResult.target_timestamp));
      ...
      case 'ai_image':
      case 'user_photo':
        updatedFields.content = parsedResult.content || parsedResult.description;
        if (parsedResult.meaning) updatedFields.meaning = parsedResult.meaning;
        break;
      case 'naiimag': {
        ...
        if (newPrompt && typeof newPrompt === 'string' && originalMessage.prompt !== newPrompt) {
          promptChanged = true;
        } else {
          newPrompt = originalMessage.prompt;
          newFullPrompt = originalMessage.fullPrompt;
          newImageUrl = originalMessage.imageUrl;
        }
        ...
        updatedFields.prompt = newPrompt;
        updatedFields.imageUrl = newImageUrl;
        updatedFields.fullPrompt = newFullPrompt;
        break;
      }
      case 'googleimag': {
        ...
        if (googlePrompt && typeof googlePrompt === 'string' && originalMessage.prompt !== googlePrompt) {
          googlePromptChanged = true;
        } else {
          googlePrompt = originalMessage.prompt;
          googleFullPrompt = originalMessage.fullPrompt;
          googleImageUrl = originalMessage.imageUrl;
        }
        ...
        if (googlePromptChanged) {
          ...
          try {
            ...
            await showCustomAlert("成功", "图片已根据提示词重新生成！");
          } catch (error) {
        ...
        updatedFields.prompt = googlePrompt;
        updatedFields.imageUrl = googleImageUrl;
        updatedFields.fullPrompt = googleFullPrompt;
        break;
      }
      case 'sticker': {
        let found = false;
        ...
        if (parsedResult.meaning) {
          ...
          if (sticker) {
            updatedFields.content = sticker.url;
            updatedFields.meaning = sticker.name;
            console.log("导演/编辑模式保存(Sticker): 使用了 meaning 查找");
            found = true;
          } else {
            updatedFields.content = `[表情: ${parsedResult.meaning}]`;
        ...
        if (!found && parsedResult.url) {
          updatedFields.content = parsedResult.url;
          ...
          if (parsedResult.meaning && (!stickerByURL || stickerByURL.name === parsedResult.meaning)) {
            updatedFields.meaning = parsedResult.meaning;
          } else if (stickerByURL) {
            updatedFields.meaning = stickerByURL.name;
          } else {
            updatedFields.meaning = '未知表情';
          }
          console.log("导演/编辑模式保存(Sticker): 使用了 URL，最终 meaning:", updatedFields.meaning);
          found = true;
        }
        ...
        if (!found && parsedResult.content && typeof parsedResult.content === 'string' && STICKER_REGEX.test(parsedResult.content)) {
          ...
          updatedFields.meaning = sticker ? sticker.name : '未知表情';
          console.log("导演/编辑模式保存(Sticker): 将 content 视为 URL，查找到 meaning:", updatedFields.meaning);
          found = true;
        }

        if (!found) {
          console.error("导演/编辑模式保存(Sticker): 指令无效或缺少必要字段 (meaning/url/content):", parsedResult);
          await showCustomAlert('保存失败', '表情消息缺少必要字段，已停止保存。');
          return;
        }
        break;
      }
      case 'transfer':
        updatedFields.amount = parsedResult.amount;
        updatedFields.note = parsedResult.note;
        break;
      case 'share_link':
        updatedFields.title = parsedResult.title;
        updatedFields.description = parsedResult.description;
        updatedFields.source_name = parsedResult.source_name;
        updatedFields.content = parsedResult.content;
        break;
      default:
        Object.assign(updatedFields, parsedResult);
        delete updatedFields.id;
        delete updatedFields.timestamp;
        delete updatedFields.createdAt;
        delete updatedFields.chatId;
        break;
    }

    updatedFields.timestamp = originalMessage.timestamp;

    if (window.messageStore?.updateMessage) {
      await window.messageStore.updateMessage(chat.id, originalMessage.timestamp, currentMessage => {
        Object.assign(currentMessage, updatedFields);
        return currentMessage;
      });
    } else {
      Object.assign(originalMessage, updatedFields);
      await db.chats.put(chat);
    }
    ...
    if (state.currentMessages) {
      const currentMemoryMessage = state.currentMessages.find(m => Number(m.timestamp) === normalizedTimestamp);
      if (currentMemoryMessage) Object.assign(currentMemoryMessage, updatedFields);
    }
  ...
  window.findChatMessageByTimestamp = findChatMessageByTimestamp;
