...
// ============================================================
  ...
  function getGroupShortMemoryCount(chat) {
    const candidates = [
  ...
  async function getRecentGroupContextMessages(chat, options = {}) {
    const resolvedLimit = Math.max(1, parseInt(options.limit, 10) || getGroupShortMemoryCount(chat));
    ...
    if (window.messageStore?.getRecentContextMessages) {
      return window.messageStore.getRecentContextMessages(chat.id, {
        ...
        excludeExcluded: options.excludeExcluded !== false
      });
    }

    let rows = Array.isArray(chat?.history) ? chat.history.slice() : [];
    if (options.excludeHidden !== false) {
      rows = rows.filter(m => !m.isHidden);
    }
    if (options.excludeExcluded !== false) {
      rows = rows.filter(m => !m.isExcluded);
    }
    return rows.slice(-resolvedLimit);
  }
...
${longTimeNoSee ? `【重要提示】你们已经很久没聊天了！你【必须】将本次行动的重点放在使用 'text' 指令给用户发消息，主动开启一个新的、有趣的话题来重新建立联系。绝对不要只是点赞或评论动态，那会显得你很冷漠！` : ''}` : ''}
        ...
        3.  **【行为多样性指南 (至关重要)】**:
            ...
            - 如果上次是 sing_song（唱歌），这次【必须】换别的类型；唱歌是稀有事件，不要连续触发。
        4.  **【行为组合指南 (最高级技巧)】**:
       ...
       - **使用亲属卡购物**:  '[{"type": "buy_item", "item_name": "商品名称", "price": 价格(数字), "reason": "购买理由/想法"}]'
        -   **给用户唱首歌**: '{"type": "sing_song", "title": "歌名", "prompt": "风格描述", "lyrics": "[Verse]...\\n[Chorus]..."}'
           - 仅在用户很久没收到你的主动互动、且氛围合适时用，不要滥用
           - 一次行动只能唱一首，不要连续唱
           - 唱完后可以不配 text 消息（sing_song 本身就会发一条系统提示）
...
`;
    ...
    try {
      ...
      for (const action of processedActions) {
        ...
        switch (action.type) {
          ...
          case 'sing_song': {
            // 【2026-07-21 新增】后台活动里 AI 主动给用户唱首歌
            // 流程跟 request_song 一样：generateSong → 存 IndexedDB → 推消息 → 自动播放
            if (!window.AIMusic || typeof window.AIMusic.generateSong !== 'function') {
              console.warn(`[BG] sing_song 跳过: AIMusic 未加载`);
              break;
            }
            const songPrompt = String(action.prompt || '').trim();
            if (!songPrompt) {
              console.warn(`[BG] sing_song 缺 prompt:`, action);
              break;
            }
            const songLyrics = typeof action.lyrics === 'string' ? action.lyrics : '';
            const useVoiceSample = action.useVoiceSample !== false;
            const instrumental = action.instrumental === true;
            const songTitle = String(action.title || action.name || 'AI 原创').trim() || 'AI 原创';
            ...
            try {
              ...
              // 复用 ai-response.js 的 IndexedDB 持久化（走全局函数）
              const persistResult = typeof window.persistAiSongBlob === 'function'
                ? await window.persistAiSongBlob(songId, {
                    chatId: chatId, blob, title: songTitle, prompt: songPrompt,
                    lyrics: songLyrics, instrumental, useVoiceSample, createdAt: Date.now()
                  })
                : false;
              if (!persistResult) {
                console.warn('[BG] sing_song IndexedDB 持久化失败（persistAiSongBlob 不可用）');
              }
              if (window.MusicPlayer && typeof window.MusicPlayer.addAiSongActionMessage === 'function') {
                await window.MusicPlayer.addAiSongActionMessage({
                  chatId: chatId, songId, title: songTitle, blob
                });
              }
              let addedIndex = -1;
              ...
              console.log(`[BG] 角色 "${chat.name}" 主动唱了《${songTitle}》`);
            } catch (songErr) {
              console.error('[BG] sing_song 失败:', songErr);
            }
            break;
          }
  ...
  async function triggerProactiveMessage(chatId) {
    ...
    // 【2026-07-18 改】主动消息用主 API——跟正常聊天一个模型，确保质量
    // 之前优先后台 API 会有"用次级模型发主动消息"的问题，主动消息质量应该跟正常对话一样
    const proxyUrl = state.apiConfig.proxyUrl;
    const apiKey = state.apiConfig.apiKey;
    const model = state.apiConfig.model;

    if (!proxyUrl || !apiKey || !model) {
      console.warn('[Proactive] 主 API 未配置，跳过。');
      return;
    }
    ...
    try {
      // ===== 复用 triggerInactiveAiAction 的核心上下文构造 =====
      // 短期历史（条数跟用户设置走；2026-07-21 加 parseInt 跟正常聊天对齐，避免字符串 maxMemory 出错）
...
`;
      ...
      // 长期记忆（双源注入：日记为主 + 向量为辅；仿照正常聊天 ai-response.js:8023-8050）
      // 【2026-07-21 改】向量记忆加 queryText，触发动态向量检索（之前只给核心记忆，等于摆设）
      const memoryMode = chat.settings.memoryMode || (chat.settings.enableStructuredMemory ? 'structured' : 'diary');
      // 构造 queryText（按向量设置里的 retrievalStrategy，跟正常聊天一致）
      ...
      try {
        ...
        console.warn('[Proactive-双源注入] 失败:', e);
        longTermMemoryContext = '# 长期记忆 (必须严格遵守)\n' + (chat.longTermMemory && chat.longTermMemory.length > 0 ? chat.longTermMemory.map(mem => `- (记录于 ${formatTimeAgo(mem.timestamp)}) ${mem.content}`).join('\n') : '- (暂无)');
      }

      // 多层对话总结（3h/6h/9h/1d/3d/7d）
      const summary3Hours = generateSummaryForTimeframe(chat, 3, 'hours');
      const summary6Hours = generateSummaryForTimeframe(chat, 6, 'hours');
      const summary9Hours = generateSummaryForTimeframe(chat, 9, 'hours');
      const summaryToday = generateSummaryForTimeframe(chat, 1, 'days');
...
- （本次主动消息不需要 buy_item，但你应该意识到这个事实。）`;
      ...
      // 【2026-07-21 改】删掉 80 字符截断的 recentContextSummary
      // history 现在作为 messages 数组完整发出去（仿照正常聊天 ai-response.js:8094-8102），
      // system 字符串里不再重复"最近的对话"，避免双重喂入 + 截断
      // 如果 filteredHistory 为空，messages 数组会空着，AI 自然知道是首次互动

      // ===== systemPrompt - 主动消息专用 v3（精简）=====
      // 【2026-07-21 v3 精简】去掉"不要问号结尾""不要空话开场白"等具体教条，
      // 避免压过角色人设——只保留调性 + 节奏 + JSON 格式
...
现在，请直接以角色身份给用户发一条消息。只输出 JSON，不要其他内容。`;
      ...
      let isGemini = String(proxyUrl || '').includes('generativelanguage.googleapis.com');
      // 【2026-07-18 改】user 消息要明确"现在是什么情境、要做什么行动"
      // 【2026-07-21 v4 精简】砍掉所有"教学"内容（7 种打开方式/不要问号结尾/不要空话开场白等），
      // 只强调两条核心：【按人设自然说话】+【自然承接不重复】——把自由发挥权还给角色。
      const minutesSinceUserRound = Math.round(minutesSinceUser || 0);
      const silenceUserMsg = minutesSinceUser !== null
        ? `你已经 ${minutesSinceUserRound} 分钟没和用户说话了（用户沉默中，没发任何消息）。`
        : `这是你和用户的第一次互动。`;
      const proactiveUserPrompt = `【系统提示：主动消息触发 - 你主动找用户】

${silenceUserMsg}

**完全按你的人设说话**——不要写成"回复"的口吻，是你在主动想起/想到/经历到什么想找他/她。

**核心要求（仅 2 条）：**
- **自然承接上文**：看 messages 数组里最近的完整对话，顺着某个具体的点展开
- **不重复**：不要重复上文已经说过的话，或者类似的微变形（换皮不换意的话也算重复）

【输出格式铁律】
你的回复【必须且只能】是一个 JSON 对象：
{"type": "text", "content": "你想对用户说的话（可以用 \\n 拆成 1-3 条气泡，每条不超过 30 字）"}

【绝对禁止】
- 输出 JSON 以外的内容（包括元描述、解释、分析、"我觉得..."等）
- 使用方括号表情/动作描写

只输出 JSON。`;

      // 【2026-07-21 改】短期历史完整喂入 messages 数组（仿照正常聊天 ai-response.js:8094-8102）
      // 之前 messagesPayload 只装一条 user prompt，history 被截到 80 字符塞 system 字符串，AI 接不上文
      const messagesForApi = filteredHistory.map(msg => ({
        role: msg.role,
        content: String(msg.content)   // 完整内容，不截断
      }));
      messagesForApi.push({
        role: 'user',
        content: proactiveUserPrompt
      });
      ...
      const chatCompletionsUrl = `${String(proxyUrl).replace(/\/+$/, '')}/chat/completions`;
      console.log('[Proactive] 请求主 API:', isGemini ? 'Gemini' : chatCompletionsUrl);
      ...
      const response = isGemini
        ...
        : useMainApiProxy
          ...
          : await fetch(chatCompletionsUrl, {
              method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(`API 失败: ${response.status} - ${JSON.stringify(errorData)}`);
      }

      const data = await response.json();
      const aiResponseContent = isGemini ? getGeminiResponseText(data) : data.choices?.[0]?.message?.content;
      if (!aiResponseContent || !aiResponseContent.trim()) {
        console.warn('[Proactive] AI 返回空');
        return;
      }
        ...
        for (const candidate of candidates) {
          try {
            ...
            if (Array.isArray(parsed)) {
              // 优先 sing_song（AI 主动选择），否则 text
              const singAction = parsed.find(item => item?.type === 'sing_song' && item.prompt);
              if (singAction) return singAction;
              const textAction = parsed.find(item => item?.type === 'text' && item.content);
              if (textAction) return textAction;
            } else if (parsed?.type === 'sing_song' && parsed.prompt) {
              return parsed;
      ...
      const action = parseProactiveTextAction(aiResponseContent);
      if (!action) {
        console.warn('[Proactive] AI 返回中找不到可解析的 text/sing_song 指令:', aiResponseContent.slice(0, 500));
        return;
      }

      // 【2026-07-21 新增】sing_song 主动唱歌分支
      if (action.type === 'sing_song') {
        if (!window.AIMusic || typeof window.AIMusic.generateSong !== 'function') {
          console.warn('[Proactive] sing_song 跳过: AIMusic 未加载');
          return;
        }
        const songPrompt = String(action.prompt || '').trim();
        if (!songPrompt) {
          console.warn('[Proactive] sing_song 缺 prompt:', action);
          return;
        }
        const songLyrics = typeof action.lyrics === 'string' ? action.lyrics : '';
        const useVoiceSample = action.useVoiceSample !== false;
        ...
        try {
          const blob = await window.AIMusic.generateSong({
            chatId: chatId, prompt: songPrompt, lyrics: songLyrics,
            useVoiceSample, instrumental, onProgress
          });
          if (!blob || typeof blob.size !== 'number' || blob.size === 0) {
            throw new Error('AI 唱歌返回空音频');
          }
          ...
          await db.chats.put(chat);
          console.log(`[Proactive] 角色 "${chat.name}" 主动唱了《${songTitle}》`);
        } catch (songErr) {
          console.error('[Proactive] sing_song 失败:', songErr);
        }
        return;
      }

      // \n 拆气泡（跟 triggerInactiveAiAction 一致）
      const contentStr = String(action.content);
      const isRawHtml = contentStr.trim().startsWith('<') && contentStr.trim().endsWith('>');
      const lines = (action.type === 'text' && !isRawHtml && contentStr.includes('\n'))
        ? contentStr.split(/\n+/).filter(line => line.trim())
        : [contentStr];

      // 落库 + 实时显示（用户在 chat 界面时立刻渲染）
      ...
      if (!isViewingThisChat) {
        chat.unreadCount = (chat.unreadCount || 0) + lines.length;
        if (typeof showNotification === 'function') {
          showNotification(chatId, lines[0]);
        }
      }

      // 主动消息使用独立时间戳，不影响后台活动冷却。
      chat.lastProactiveTimestamp = Date.now();
      await db.chats.put(chat);

      console.log(`[Proactive] ✅ 角色 "${chat.name}" 主动消息已发 (${lines.length} 条气泡)${isViewingThisChat ? '，实时渲染到 chat' : ''}`);
    } catch (error) {
      console.error(`[Proactive] triggerProactiveMessage 失败:`, error);
    } finally {
      setAvatarActingState(chatId, false);
      renderChatList();
    }
  ...
  async function triggerGroupAiAction(chatId) {
    ...
    const maxMemory = getGroupShortMemoryCount(chat);
    const recentHistory_RAW = await getRecentGroupContextMessages(chat, {
      limit: maxMemory,
      excludeHidden: true,
    ...
    console.log('[AI上下文] 实际加载消息数:', recentHistory_RAW.length);
    ...
    const recentHistory = await getRecentGroupContextMessages(chat, {
      limit: 5,
      excludeHidden: true,
    ...
    if (unclaimedPacket) {
      ...
      if (chat.settings.enableTimePerception) {
        const lastMessage = (await getRecentGroupContextMessages(chat, {
          limit: 1,
          excludeHidden: true,
          excludeExcluded: false
        }))[0];
...
`;
      ...
      if (chat.settings.linkedMemoryChatIds && chat.settings.linkedMemoryChatIds.length > 0) {
        ...
        for (const item of linkedChatsWithTimestamps) {
          ...
          const timeAgo = item.latestTimestamp > 0 ? ` (最后互动于 ${formatTimeAgo(item.latestTimestamp)})` : '';
          linkedMemoryContext += `\n## --- 来自${prefix}"${linkedChat.name}"的参考记忆${timeAgo} ---\n`;
          const linkedRecentHistory = await getRecentGroupContextMessages(linkedChat, {
            limit: memoryCount,
            excludeHidden: false,
            excludeExcluded: false
          });
          const filteredLinkedHistory = linkedRecentHistory.filter(msg => !String(msg.content).includes('已被用户删除'));
          if (filteredLinkedHistory.length > 0) {
            filteredLinkedHistory.forEach(msg => {
...
- 【绝对禁止】使用任何不在列表中的表情含义！
  ...
  // ========== 全局暴露 (2026-07-18 加) ==========
  // triggerProactiveMessage 用于后台活动 tick 在用户停留在 chat 里时强制发消息
