...
window.initEventBindingsB = function(state, db) {
    ...
    document.getElementById('clear-chat-btn').addEventListener('click', async () => {
      ...
      const message = keepCount > 0
        ? `将删除当前聊天中除最近 ${keepCount} 条外的所有消息。此操作无法恢复，确定继续吗？`
        : '此操作将永久删除此聊天的所有消息，无法恢复。确定要清空吗？';

      const confirmed = await showCustomConfirm('清空聊天记录', message, {
        confirmButtonClass: 'btn-danger'
      });
      ...
      if (confirmed) {
        try {
          ...
          renderChatList();
          showToast(keepCount > 0 ? `已保留最近 ${keepCount} 条` : '已清空聊天记录');
    ...
    document.getElementById('image-upload-input').addEventListener('change', async (event) => {
      ...
      reader.onload = async (e) => {
        ...
        const msg = {
          ...
          timestamp: Date.now(),
          imageVisionStatus: 'pending'
        };
        chat.history.push(msg);
        await db.chats.put(chat);
        appendMessage(msg, chat);
        renderChatList();
        if (typeof runVisionRecognitionForChatMessage === 'function') {
          runVisionRecognitionForChatMessage(chat.id, msg.timestamp);
        }
      };
      reader.readAsDataURL(file);
      event.target.value = null;
    ...
    document.getElementById('selection-erase-btn').addEventListener('click', async () => {
      ...
      if (confirmed) {
        const chatId = state.activeChatId;
        const messagesToDelete = window.messageStore?.getMessagesByTimestamps
          ? await window.messageStore.getMessagesByTimestamps(chatId, [...selectedMessages])
          : state.chats[chatId].history.filter(msg => selectedMessages.has(msg.timestamp));

        let deletedCount = 0;
        for (const message of messagesToDelete) {
          if (window.messageStore?.deleteMessage) {
            const ok = await window.messageStore.deleteMessage(chatId, message);
            if (ok) deletedCount++;
          }
        }
    ...
    document.getElementById('user-speak-btn').addEventListener('click', handleVideoCallUserSpeak);
    ...
    async function openSharedHistoryViewer(timestamp) {
      ...
      for (const sharedMsg of message.payload.sharedHistory) {
        try {
          ...
          if (bubbleEl) fragment.appendChild(bubbleEl);
        } catch (error) {
          window.runtimeDiag?.log?.('CHAT_RENDER_BAD_MESSAGE_SKIPPED', { chatId: sourceChat?.id, messageTimestamp: sharedMsg?.timestamp, error: error?.message || String(error) });
          console.warn('[CHAT_RENDER_BAD_MESSAGE_SKIPPED]', error, sharedMsg);
        }
      }
    ...
    document.getElementById('memory-screen-back-btn').addEventListener('click', () => {
      // 从 hub 进来的 → 返回 hub；老路径直接进的 → 返回聊天框
      if (window._memoryEntryFromHub && state.activeChatId) {
        openMemoryHub();
      } else {
        showScreen('chat-interface-screen');
      }
    ...
    // 记忆主页 / 向量记忆宫殿 的导航绑定
    document.getElementById('memory-hub-back-btn')?.addEventListener('click', () => {
      showScreen('chat-interface-screen');
    });
    document.getElementById('vector-memory-back-btn')?.addEventListener('click', () => {
      openMemoryHub();
    });
    // 顶部下拉切换模式
    document.getElementById('memory-hub-mode-select')?.addEventListener('change', async (e) => {
      if (!state.activeChatId) return;
      const chat = state.chats[state.activeChatId];
      const mode = e.target.value;
      chat.settings.memoryMode = mode;
      chat.settings.enableStructuredMemory = (mode === 'structured');
      chat.settings.enableDiaryMode = (mode === 'diary' || mode === 'hybrid');
      if (window.db) await db.chats.put(chat);
      // 重新渲染 hub
      renderMemoryHub(chat);
    });
    // 向量页右上角加号
    document.getElementById('vector-memory-add-btn')?.addEventListener('click', () => {
      if (!state.activeChatId || !window.vectorMemoryManager) return;
      const chat = state.chats[state.activeChatId];
      // 调用已有逻辑：触发 add-fragment 流程
      document.getElementById('vm-add-fragment-btn')?.click();
    });
    ...
    // 结构化记忆 Tab 切换（长存记忆页不再用 Tab，结构化/向量记忆从 memory-hub 走专属页）
    // 保留 ? 保护以防代码其它处引用
    ...
    document.getElementById('regenerate-call-btn').addEventListener('click', () => {
      if (window.switchVideoCallCameraFacingMode) {
        window.switchVideoCallCameraFacingMode();
      }
    });

    // 视频通话「模型管理」按钮 + 浮层面板 — 视频通话中的 Live2D 表情/动作控制器
    // 复用 Live2DLoader.setExpression / playMotion 公开 API, 不修改 live2d-manager.js
    // 数据读取逻辑从 live2d-manager.js 复制纯函数 (3 步 fallback: refs → 扫文件 → model3.json)
    const mmPanel = document.getElementById('model-management-panel');
    const mmCloseBtn = document.getElementById('model-management-close-btn');
    const mmExprEl = document.getElementById('model-management-expressions');
    const mmMotionEl = document.getElementById('model-management-motions');
    const mmExprEmptyEl = document.getElementById('model-management-expressions-empty');
    const mmMotionEmptyEl = document.getElementById('model-management-motions-empty');
    const mmExprResetEl = document.getElementById('model-management-expression-reset');
    const mmNoModelEl = document.getElementById('model-management-no-model');

    // 复制自 live2d-manager.js:18-32 (EXPRESSION_CN_LABELS) — 加新模型时同步维护
    const MM_EXPR_CN = {
      'cry': '流泪', 'mask': '面具', 'star_eyes': '星星眼',
      'cat_ears': '猫耳', 'cat_paw_up': '举手猫爪', 'dark_face': '黑脸',
      'action3': '动作3', 'face_sticker': '脸贴纸', 'blush': '脸红',
      'cat_tail': '猫尾巴', 'hairpin': '发卡', 'earring': '耳钉',
      'zoom': '放大',
    };
    ...
    // 复制自 live2d-manager.js:1028-1036 (readExpressionsFromData) — 读 IDB refs
    function readMmExpressionsFromRefs(data) {
      const a = data && data.config && data.config.refs && data.config.refs.Expressions;
    ...
    // 复制自 live2d-manager.js:1011-1026 (readMotionsFromData) — 读 IDB refs
    function readMmMotionsFromRefs(data) {
      const m = data && data.config && data.config.refs && data.config.refs.Motions;
      if (!m || typeof m !== 'object') return [];
      const out = [];
      for (const g of Object.keys(m)) {
        if (Array.isArray(m[g])) {
          m[g].forEach((it, i) => {
            out.push({ group: g, index: i, name: (it && (it.Name || it.name)) || (g + '-' + i) });
          });
        }
      }
      return out;
    ...
    // 兜底 1: 扫 data.files 里 .exp3.json / .motion3.json (refs 为空时用)
    function discoverMmFromFiles(data, ext) {
      if (!data || !data.files) return [];
      const out = [];
      const wantExt = '.' + ext;
      for (const [filePath] of data.files.entries()) {
        const lower = filePath.toLowerCase();
        if (!lower.endsWith(wantExt)) continue;
        const base = filePath.substring(filePath.lastIndexOf('/') + 1, filePath.length - wantExt.length);
        if (ext === 'exp3.json') {
          out.push({ id: base, name: base });
        } else {
          // motion 命名约定: group_index.motion3.json, 兜底 group_0
          const m = base.match(/^(.+?)[-_](\d+)$/);
          if (m) out.push({ group: m[1], index: parseInt(m[2], 10), name: base });
          else out.push({ group: base, index: 0, name: base });
        }
      }
      return out;
    }

    // 兜底 2: 读 data.files 里的 model3.json 解析 (Live2DConfig.parseModel3Json)
    async function readMmFromModel3Json(data) {
      if (!data || !data.files || !data.modelPath) return { expressions: [], motions: [] };
      if (!window.Live2DConfig || !window.Live2DConfig.parseModel3Json) return { expressions: [], motions: [] };
      const blob = data.files.get(data.modelPath);
      if (!blob || typeof blob.text !== 'function') return { expressions: [], motions: [] };
      let model3Json;
      try { model3Json = JSON.parse(await blob.text()); } catch (e) { return { expressions: [], motions: [] }; }
      let config;
      try { config = window.Live2DConfig.parseModel3Json(model3Json, data.modelPath); } catch (e) { return { expressions: [], motions: [] }; }
      const actions = (config && Array.isArray(config.actions)) ? config.actions : [];
      return {
        expressions: actions.filter(a => a.kind === 'expression').map(a => ({ id: a.expressionId || a.name, name: a.name })),
        motions: actions.filter(a => a.kind === 'motion').map(a => ({ group: a.group, index: typeof a.index === 'number' ? a.index : 0, name: a.name })),
      };
    }

    async function readMmExpressions(data) {
      let list = readMmExpressionsFromRefs(data);
      if (list.length > 0) return list;
      list = discoverMmFromFiles(data, 'exp3.json');
      if (list.length > 0) return list;
      const { expressions } = await readMmFromModel3Json(data);
      return expressions;
    }
    async function readMmMotions(data) {
      let list = readMmMotionsFromRefs(data);
      if (list.length > 0) return list;
      list = discoverMmFromFiles(data, 'motion3.json');
      if (list.length > 0) return list;
      const { motions } = await readMmFromModel3Json(data);
      return motions;
    }

    function renderMmExpressions(expressions, defaultId) {
      if (!mmExprEl) return;
      if (!expressions || expressions.length === 0) {
        mmExprEl.innerHTML = '';
        mmExprEl.style.display = 'none';
        if (mmExprEmptyEl) mmExprEmptyEl.style.display = 'block';
        return;
      }
      mmExprEl.style.display = 'flex';
      if (mmExprEmptyEl) mmExprEmptyEl.style.display = 'none';
      const html = expressions.map(e => {
        const cn = MM_EXPR_CN[e.id];
        const label = cn && cn !== e.name ? (cn + ' ' + e.name) : e.name;
        const isActive = (defaultId && e.id === defaultId) ? ' model-management-action-btn-active' : '';
        return '<button type="button" class="model-management-action-btn' + isActive + '" ' +
          'data-action="expression" data-id="' + mmEsc(e.id) + '" title="' + mmEsc(e.name) + '">' +
          mmEsc(label) + '</button>';
      }).join('');
      mmExprEl.innerHTML = html;
    }
    function renderMmMotions(motions) {
      if (!mmMotionEl) return;
      if (!motions || motions.length === 0) {
        mmMotionEl.innerHTML = '';
        mmMotionEl.style.display = 'none';
        if (mmMotionEmptyEl) mmMotionEmptyEl.style.display = 'block';
        return;
      }
      mmMotionEl.style.display = 'flex';
      if (mmMotionEmptyEl) mmMotionEmptyEl.style.display = 'none';
      const html = motions.map(m => {
        return '<button type="button" class="model-management-action-btn" ' +
          'data-action="motion" data-group="' + mmEsc(m.group) + '" data-index="' + m.index + '" ' +
          'title="' + mmEsc(m.group) + ' · ' + mmEsc(m.name) + '">' +
          mmEsc(m.name) + '</button>';
      }).join('');
      mmMotionEl.innerHTML = html;
    }

    function mmGetCanvasAndChat() {
      const canvas = document.getElementById('live2d-canvas');
      // 视频通话中的 chat 由 videoCallState.activeChatId 决定 (window 暴露, 权威来源; doInitiateCall:630 设置)
      // state.activeChatId 可能是另一个当前打开的聊天窗口, 不是正在通话的那个人 → 之前 defaultExpression 读错聊天返回空
      let chatId = null;
      try {
        if (window.videoCallState && window.videoCallState.activeChatId) {
          chatId = window.videoCallState.activeChatId;
        }
      } catch (e) { /* ignore */ }
      // 兜底: videoCallState 不可用 / 指向不存在的 chat 时, 回退 state.activeChatId
      if (!chatId || !state || !state.chats || !state.chats[chatId]) {
        chatId = (state && state.activeChatId) ? state.activeChatId : null;
      }
      const chat = (chatId && state && state.chats) ? state.chats[chatId] : null;
      return { canvas, chat };
    }

    // 解析 modelId + defaultExpression (3 层 fallback: appearance → per-chat → 全局 → localStorage)
    async function getMmCurrentModelIdAndDefault() {
      const { chat } = mmGetCanvasAndChat();
      let modelId = null, defaultExpression = '';
      try {
        if (window.Live2DStorage) {
          if (chat && chat.id) {
            ...
            if (appearance && appearance.modelId) {
              ...
              modelId = await window.Live2DStorage.getActiveModelIdForChat(chat.id);
            }
          }
          if (!modelId) {
            modelId = await window.Live2DStorage.getActiveModelId();
          }
        }
      } catch (e) { /* ignore */ }
      if (!modelId) {
        try { modelId = localStorage.getItem('live2d.activeModelId'); } catch (e) { /* ignore */ }
      }
      // defaultExpression 兜底: 主 chat 没存 default 时, 扫所有 chat 的 appearance 找 modelId 匹配 + defaultExpression 非空的
      // 场景: 视频通话 chat 可能没走过"形象调试台→使用此形象"流程, 但同模型的另一个 chat 存过
      if (!defaultExpression && modelId && window.Live2DStorage) {
        try {
          const chats = (state && state.chats) ? state.chats : {};
          for (const cid of Object.keys(chats)) {
            const app = await window.Live2DStorage.getVideoCallAppearance(cid);
            if (app && app.modelId === modelId && app.defaultExpression) {
      ...
      return { modelId, defaultExpression };
    }

    async function loadAndRenderMmPanel() {
      if (mmNoModelEl) mmNoModelEl.style.display = 'none';
      const { modelId, defaultExpression } = await getMmCurrentModelIdAndDefault();
      if (!modelId || !window.Live2DStorage) {
        if (mmNoModelEl) { mmNoModelEl.style.display = 'block'; mmNoModelEl.textContent = '尚未加载 Live2D 模型'; }
        if (mmExprEl) mmExprEl.style.display = 'none';
        if (mmMotionEl) mmMotionEl.style.display = 'none';
        if (mmExprEmptyEl) mmExprEmptyEl.style.display = 'none';
        if (mmMotionEmptyEl) mmMotionEmptyEl.style.display = 'none';
        if (mmExprResetEl) { mmExprResetEl.disabled = true; mmExprResetEl.title = '未加载模型'; }
        return;
      }
      let data = null;
      try { data = await window.Live2DStorage.getModel(modelId); } catch (e) { /* ignore */ }
      if (!data) {
        if (mmNoModelEl) { mmNoModelEl.style.display = 'block'; mmNoModelEl.textContent = '模型数据读取失败 (id=' + modelId + ')'; }
        return;
      }
      const expressions = await readMmExpressions(data);
      const motions = await readMmMotions(data);
      renderMmExpressions(expressions, defaultExpression);
      renderMmMotions(motions);
      if (mmExprResetEl) {
        if (defaultExpression) {
          mmExprResetEl.disabled = false;
          mmExprResetEl.title = '恢复默认表情: ' + defaultExpression;
        } else {
          mmExprResetEl.disabled = true;
          mmExprResetEl.title = '未设置默认表情';
        }
      }
      // ↺ 默认 路径 1 依赖: 面板首次打开时, 拿当前 canvas 所有 parameter 快照
      // 调试台 live2d-manager.js:1376 saveInitialParamValues 同样思路, 但它在挂载时拍
      // 我们在面板打开时拍, 时机等价 (defaultExpression 已应用 / 表情按钮已渲染完成)
      try {
        const c = document.getElementById('live2d-canvas');
        if (c && c._live2dModel) {
          const m = c._live2dModel;
          ...
          if (core && typeof core.getParameterCount === 'function' && typeof core.getParameterValueByIndex === 'function') {
            ...
            if (mmExprResetEl) { mmExprResetEl.disabled = true; mmExprResetEl.title = '快照失败, SDK 不支持'; }
          }
        } else {
          console.warn('[ModelManage] 快照失败: canvas 或 _live2dModel 不存在');
          if (mmExprResetEl) { mmExprResetEl.disabled = true; mmExprResetEl.title = 'Live2D 未挂载'; }
        }
      } catch (e) {
        console.warn('[ModelManage] 快照异常:', e);
        if (mmExprResetEl) { mmExprResetEl.disabled = true; mmExprResetEl.title = '快照异常: ' + (e.message || e); }
      }
    }

    // 事件委托 (一次绑定, 处理 expression / motion / reset 按钮 — 按钮是动态注入的)
    if (mmPanel) {
      mmPanel.addEventListener('click', function (ev) {
        const btn = ev.target.closest('[data-action]');
        if (!btn) return;
        const action = btn.getAttribute('data-action');
        // 一次性诊断: 看 handler 触发 + canvas/reset btn 状态
        if (action === 'expression-reset') {
          try {
            const cNow = document.getElementById('live2d-canvas');
            console.log('[ModelManage] ↺ 默认 handler 触发 | resetBtn.disabled='
              + (mmExprResetEl ? mmExprResetEl.disabled : '?')
              + ' | canvas存在=' + !!cNow
              + ' | canvas._live2dModel=' + !!(cNow && cNow._live2dModel)
              + ' | 快照=' + (cNow && cNow._live2dInitialParams ? cNow._live2dInitialParams.size : '无')
              + ' | event.target=' + (ev.target && ev.target.tagName + (ev.target.id ? '#' + ev.target.id : '')));
          } catch (e) {}
        }
        const { canvas } = mmGetCanvasAndChat();
        if (!canvas || !canvas._live2dModel) {
          console.warn('[ModelManage] Live2D canvas 未挂载, 忽略点击');
          return;
        }
        try {
          if (action === 'expression') {
            const id = btn.getAttribute('data-id');
            if (window.Live2DLoader && window.Live2DLoader.setExpression) {
              const r = window.Live2DLoader.setExpression(canvas, id);
              if (r && typeof r.catch === 'function') r.catch(function (e) { console.warn('[ModelManage] setExpression reject:', e); });
            }
            if (mmExprEl) {
              mmExprEl.querySelectorAll('[data-action="expression"]').forEach(b => b.classList.remove('model-management-action-btn-active'));
              btn.classList.add('model-management-action-btn-active');
            }
          } else if (action === 'motion') {
            const group = btn.getAttribute('data-group');
            const index = parseInt(btn.getAttribute('data-index'), 10);
            if (window.Live2DLoader && window.Live2DLoader.playMotion) {
              window.Live2DLoader.playMotion(canvas, group, Number.isFinite(index) ? index : 0);
            }
            btn.classList.add('model-management-action-btn-active');
            setTimeout(function () { btn.classList.remove('model-management-action-btn-active'); }, 400);
          } else if (action === 'expression-reset') {
            // ↺ 默认: 抄 live2d-manager.js 调试台 resetExpressionParams 的实现思路
            //   调试台做法: 挂载时 snapshot 所有 parameter 初始值 → reset 时 setParameterValueByIndex 还原
            //   视频通话没存快照, 用两路兜底:
            //     1) 如果当前 canvas 有 _live2dInitialParams 快照 (调试台存的或我们后面自己存的), 用它重置
            //     2) 否则 setExpression(canvas, '') — 走库的 expressionManager.resetExpression, fork 上返 false
            //     3) 都没有就尝试用 appearance.defaultExpression 重新应用 (跟用户上次选的默认对齐)
            let applied = false;
            // 路径 1: 快照
            try {
              if (canvas && canvas._live2dModel && canvas._live2dInitialParams
                  && canvas._live2dInitialParams.size > 0) {
                const m = canvas._live2dModel;
                const im = m.internalModel;
                const core = im && (im.coreModel || im._model || (im.coreModel && im.coreModel._model));
                if (core && typeof core.setParameterValueByIndex === 'function') {
                  let n = 0;
                  canvas._live2dInitialParams.forEach(function (val, idx) {
                    try { core.setParameterValueByIndex(idx, val); n++; } catch (e) {}
                  });
                  if (n > 0) {
                    applied = true;
                    console.log('[ModelManage] ↺ 默认: 路径 1 (快照) 应用了 ' + n + ' 个参数');
                  }
                }
              }
            } catch (e) { console.warn('[ModelManage] ↺ 默认 路径 1 失败:', e); }
            // 路径 2: 库 resetExpression
            if (!applied) {
              try {
                if (window.Live2DLoader && window.Live2DLoader.setExpression) {
                  const r = window.Live2DLoader.setExpression(canvas, '');
                  Promise.resolve(r).then(function (ok) {
                    if (ok !== false) {
                      applied = true;
                      console.log('[ModelManage] ↺ 默认: 路径 2 (库 reset) ok=' + ok);
                    } else {
                      console.warn('[ModelManage] ↺ 默认 路径 2: 库不支持 reset, 走路径 3');
                    }
                  }).catch(function (e) { console.warn('[ModelManage] ↺ 默认 路径 2 reject:', e); });
                }
              } catch (e) { console.warn('[ModelManage] ↺ 默认 路径 2 抛错:', e); }
            }
            // 路径 3 (同步): 读 appearance.defaultExpression 重新应用 — 用户上次选的默认
            if (!applied) {
              try {
                getMmCurrentModelIdAndDefault().then(function (m) {
                  const def = m.defaultExpression;
                  if (def && window.Live2DLoader && window.Live2DLoader.setExpression) {
                    const r = window.Live2DLoader.setExpression(canvas, def);
                    Promise.resolve(r).then(function (ok) {
                      console.log('[ModelManage] ↺ 默认 路径 3: 重新应用 defaultExpression=' + def + ' ok=' + ok);
                    });
                    if (mmExprEl) {
                      mmExprEl.querySelectorAll('[data-action="expression"]').forEach(function (b) {
                        if (b.getAttribute('data-id') === def) b.classList.add('model-management-action-btn-active');
                        else b.classList.remove('model-management-action-btn-active');
                      });
                    }
                  } else {
                    console.warn('[ModelManage] ↺ 默认: 三条路径都不可用, 该模型不支持 reset, 也未设 defaultExpression');
                  }
                });
              } catch (e) { console.warn('[ModelManage] ↺ 默认 路径 3 抛错:', e); }
            }
          }
        } catch (e) {
          console.warn('[ModelManage] click failed:', e);
        }
      });
    }

    document.getElementById('model-manage-btn').addEventListener('click', function () {
      if (!mmPanel) return;
      const wasOpen = mmPanel.classList.contains('visible');
      mmPanel.classList.toggle('visible');
      if (!wasOpen) {
        // 打开时异步加载并渲染 (关闭时复用上次渲染结果, 避免重复 IO)
        loadAndRenderMmPanel().catch(function (e) { console.warn('[ModelManage] load failed:', e); });
      }
    });
    ...
    if (mmCloseBtn) {
      mmCloseBtn.addEventListener('click', function () {
        if (!mmPanel) return;
        mmPanel.classList.remove('visible');
      });
    }
