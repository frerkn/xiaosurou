  ...
  async function handleGenerateShoppingItems() {
    ...
    if (typeof resolveApiSlotConfig === 'function') {
      ...
      if (secondaryConfig && secondaryConfig.proxyUrl && secondaryConfig.apiKey && secondaryConfig.model) {
        ...
        apiConfig = await resolveApiSlotConfig('main');
      }
    } else {
      const fallbackConfig = state.apiConfig || {};
      useSecondaryApi = fallbackConfig.secondaryProxyUrl && fallbackConfig.secondaryApiKey && fallbackConfig.secondaryModel;
      apiConfig = useSecondaryApi
        ? {
            proxyUrl: fallbackConfig.secondaryProxyUrl,
            apiKey: fallbackConfig.secondaryApiKey,
            model: fallbackConfig.secondaryModel
          }
        : fallbackConfig;
    }
...
现在，请根据以上【所有上下文信息】，开始为"${chat.name}"生成这组【与角色高度相关】的商品数据。`;
    ...
    try {
      ...
      const response = isGemini ?
        ...
        await fetch(`${(useSecondaryApi ? proxyUrl.replace(/\/+$/, '') : proxyUrl)}/${useSecondaryApi ? 'chat/completions' : 'v1/chat/completions'}`, {
          method: 'POST',
          headers: {
