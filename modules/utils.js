...
// 默认图标 URL 全部清空（原 postimg/imgur/美团云 URL 已失效，避免显示蓝粑粑裂图）
// 初始界面用空 src + CSS 兜底（display:none + SVG 默认图标），用户后来传图或导入美化再覆盖
...
// 默认头像清空（原 postimg URL 已失效，避免蓝粑粑）
const defaultAvatar = '';
const defaultMyGroupAvatar = '';
const defaultGroupMemberAvatar = '';
