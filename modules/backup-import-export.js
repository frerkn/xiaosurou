...
// ============================================================
  ...
  const BACKUP_IMAGE_DATA_URL_RE = /^data:image\/[a-zA-Z0-9.+-]+;base64,/;

  function canUploadBackupImagesToImgBB() {
    return !!(
      state &&
      state.apiConfig &&
      state.apiConfig.imgbbEnable &&
      state.apiConfig.imgbbApiKey &&
      typeof uploadImageToImgBB === 'function'
    );
  }

  function isBackupLocalImage(value) {
    return typeof value === 'string' && BACKUP_IMAGE_DATA_URL_RE.test(value);
  }

  function syncMigratedRecordToState(tableName, record) {
    if (!record || !state) return;

    if (tableName === 'globalSettings' && record.id === 'main' && state.globalSettings) {
      Object.assign(state.globalSettings, record);
      return;
    }

    if (tableName === 'apiConfig' && record.id === 'main' && state.apiConfig) {
      Object.assign(state.apiConfig, record);
      return;
    }

    if (tableName === 'qzoneSettings' && record.id === 'main' && state.qzoneSettings) {
      Object.assign(state.qzoneSettings, record);
      return;
    }

    if (tableName === 'chats' && record.id && state.chats && state.chats[record.id]) {
      Object.assign(state.chats[record.id], record);
    }
  }

  function updateBackupImageMigrationStatus(stats, tableName = '') {
    ...
    modalBody.innerHTML = `
      ...
      <p style="text-align:center;">
        正在把本地图片上传到图床...<br>
        ${tableName ? `当前数据表：<b>${tableName}</b><br>` : ''}
        已发现 ${stats.found} 张，成功 ${stats.uploaded} 张，失败 ${stats.failed} 张
      </p>
      <p style="text-align:center;font-size:12px;color:var(--text-secondary,#888);">
        上传失败或未配置图床时，会保留原本图片继续导出。
      </p>
    `;
  }

  async function migrateBackupImagesToImgBB(options = {}) {
    const stats = {
      skipped: false,
      limitReached: false,
      found: 0,
    ...
    if (!canUploadBackupImagesToImgBB()) {
      stats.skipped = true;
      return stats;
    }

    const allTableNames = db.tables.map(t => t.name);
    const requestedTableNames = Array.isArray(options.tableNames) && options.tableNames.length > 0
      ? options.tableNames
      : allTableNames;
    const tableNames = [...new Set(requestedTableNames)].filter(name => allTableNames.includes(name));
    const uploadCache = new Map();
    const UPLOAD_THROTTLE_MS = 250;
    const startedAt = Date.now();
    const maxUploads = Math.max(0, Number(options.maxUploads ?? 1));
    const maxMillis = Math.max(1000, Number(options.maxMillis ?? 6000));
    const uploadTimeoutMs = Math.max(1000, Math.min(Number(options.uploadTimeoutMs ?? 6000), maxMillis));
    const maxRecordsScanned = Math.max(1, Number(options.maxRecordsScanned ?? 200));
    let loadingToast = null;
    const shouldScanLocalStorage = !Array.isArray(options.tableNames);
    const showStatusInModal = !options.silent && options.showStatusInModal === true;
    ...
    if (!options.silent && typeof showToast === 'function') {
      loadingToast = showToast('正在检查本地图片...', 'loading');
    }

    const uploadWithCache = async (dataUrl) => {
      if (uploadCache.has(dataUrl)) {
        return uploadCache.get(dataUrl);
      }

      const elapsedMs = Date.now() - startedAt;
      if (stats.uploaded + stats.failed >= maxUploads || elapsedMs >= maxMillis) {
        stats.limitReached = true;
      ...
      const uploadPromise = (async () => {
        ...
        const timeoutMs = Math.max(1, Math.min(uploadTimeoutMs, maxMillis - elapsedMs));
        const timeoutId = controller
          ? setTimeout(() => controller.abort(), timeoutMs)
          : null;

        try {
          const url = await uploadImageToImgBB(dataUrl, controller ? { signal: controller.signal } : undefined);
          await new Promise(resolve => setTimeout(resolve, UPLOAD_THROTTLE_MS));
          if (url && url !== dataUrl && !String(url).startsWith('data:')) {
            return url;
          }
          return null;
        } catch (error) {
          console.warn('[Backup ImgBB] image upload failed:', error);
          return null;
        } finally {
    ...
    const migrateNode = async (node) => {
      if (stats.limitReached) {
        return { value: node, changed: false };
      }

      if (isBackupLocalImage(node)) {
        stats.found++;
        const wasLimitReached = stats.limitReached;
        const uploadedUrl = await uploadWithCache(node);
        if (uploadedUrl) {
          stats.uploaded++;
          return { value: uploadedUrl, changed: true };
        }
        if (stats.limitReached && !wasLimitReached) {
          return { value: node, changed: false };
        }
        stats.failed++;
        return { value: node, changed: false };
      ...
      if (Array.isArray(node)) {
        ...
        for (let i = 0; i < node.length; i++) {
          ...
          const result = await migrateNode(node[i]);
          if (result.changed) {
            node[i] = result.value;
            changed = true;
          }
      ...
      if (node && typeof node === 'object') {
        ...
        for (const key of Object.keys(node)) {
          if (stats.limitReached) break;
          const result = await migrateNode(node[key]);
          if (result.changed) {
    ...
    try {
      if (showStatusInModal) updateBackupImageMigrationStatus(stats);
      ...
      for (const tableName of tableNames) {
        ...
        while (offset < totalCount) {
          if (stats.limitReached) break;
          const records = await table.offset(offset).limit(BATCH_SIZE).toArray();

          for (const record of records) {
            if (stats.limitReached) break;
            stats.recordsScanned++;
            if (stats.recordsScanned > maxRecordsScanned) {
              stats.limitReached = true;
              stats.skippedByLimit++;
              break;
            }
            if (showStatusInModal) updateBackupImageMigrationStatus(stats, tableName);
            const result = await migrateNode(record);
            if (result.changed) {
              await table.put(record);
              syncMigratedRecordToState(tableName, record);
      ...
      if (shouldScanLocalStorage) {
        for (let i = 0; i < localStorage.length; i++) {
          if (stats.limitReached) break;
          const key = localStorage.key(i);
          if (!key || !key.startsWith('couple')) continue;

          const rawValue = localStorage.getItem(key);
          if (!rawValue) continue;

          if (isBackupLocalImage(rawValue)) {
            const result = await migrateNode(rawValue);
            if (result.changed) {
          ...
          try {
            ...
            const result = await migrateNode(parsedValue);
            if (result.changed) {
              localStorage.setItem(key, JSON.stringify(result.value));
              stats.recordsUpdated++;
            }
          } catch (error) {
            // Non-JSON localStorage values are left as-is.
          }
        }
      }

      console.log('[Backup ImgBB] migration finished:', stats);
      return stats;
    } finally {
      if (loadingToast) {
  ...
  async function runPreBackupImageMigration(options = {}) {
    try {
      const stats = await migrateBackupImagesToImgBB(options);
      if (stats && stats.limitReached) {
        console.warn('[Backup ImgBB] migration limit reached, continuing backup:', stats);
        if (!options.silent && typeof showToast === 'function') {
          showToast('图床预上传已达到安全上限，继续导出备份', 'info');
        }
      }
      return stats;
    } catch (error) {
      console.warn('[Backup ImgBB] pre-backup migration skipped after error:', error);
      if (!options.silent && typeof showToast === 'function') {
        showToast('图床预上传出错，已跳过并继续备份', 'warning');
      }
      return { skipped: true, error: error && error.message ? error.message : String(error) };
    }
  }
  ...
  async function exportBackup() {
    try {
      await runPreBackupImageMigration();
    ...
    try {
      ...
      try {
        ...
        if (restoredApiConfig) {
          ...
          // 统一归一化 TTS 配置（兼容旧 Minimax 字段并清理旧 localStorage）
          if (window.TTSService && typeof window.TTSService.normalizeTtsConfig === 'function') {
            restoredApiConfig.tts = window.TTSService.normalizeTtsConfig(restoredApiConfig);
            await db.apiConfig.put(restoredApiConfig);
          }
  ...
  async function showAdvancedExportImportModal() {
    ...
    const dataCategories = {
      '聊天与消息': ['chats'],
      '空间与社交': ['qzoneSettings', 'qzonePosts', 'qzoneAlbums', 'qzonePhotos', 'qzoneGroups'],
  ...
  async function exportSelectedTables(tables, categoryNames, selectedChatIds = []) {
    ...
    try {
      await runPreBackupImageMigration({ tableNames: tables });
  ...
  async function uploadToGitHub(isSilent = false) {
    ...
    try {
      await runPreBackupImageMigration({ silent: isSilent });
