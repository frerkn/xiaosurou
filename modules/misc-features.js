...
// ============================================================
  ...
  async function showWaimaiDetails(timestamp) {
    ...
    if (window.messageStore?.getMessageByTimestamp) {
      message = await window.messageStore.getMessageByTimestamp(chat.id, timestamp);
    }
    if (!message) {
      message = chat.history.find(m => m.timestamp === timestamp);
    }
  ...
  async function handleWaimaiResponse(originalTimestamp, choice) {
    ...
    if (window.messageStore?.getMessageByTimestamp) {
      originalMessage = await window.messageStore.getMessageByTimestamp(chat.id, originalTimestamp);
    }
    if (!originalMessage) {
      originalMessage = chat.history.find(m => m.timestamp === originalTimestamp);
    }
    ...
    if (window.messageStore?.updateMessage) {
      await window.messageStore.updateMessage(chat.id, originalTimestamp, {
        status: originalMessage.status,
        paidBy: originalMessage.paidBy
      });
    }
    ...
    if (window.messageStore?.addMessageToChat) {
      await window.messageStore.addMessageToChat(chat.id, systemNote);
    } else {
    ...
    for (const anno of announcements) {
      ...
      if (originalMessage) {
        ...
        try {
          ...
          if (messageBubbleEl) wrapper.appendChild(messageBubbleEl);
        } catch (error) {
          window.runtimeDiag?.log?.('CHAT_RENDER_BAD_MESSAGE_SKIPPED', { chatId: chat?.id, messageTimestamp: originalMessage?.timestamp, error: error?.message || String(error) });
          console.warn('[CHAT_RENDER_BAD_MESSAGE_SKIPPED]', error, originalMessage);
        }
      } else {
...
// ============================================================
  ...
  const DEFAULT_BUTTON_ORDER = [
    'open-sticker-panel-btn', 'upload-image-btn',
    'transfer-btn', 'voice-record-btn', 'send-waimai-request-btn',
    'video-call-btn', 'group-video-call-btn', 'voice-call-btn', 'group-voice-call-btn', 'send-poll-btn',
    'share-link-btn', 'share-location-btn',
    'pat-btn', 'edit-last-response-btn',
...
`;
  ...
  // 待办视图模式：'day' = 单天（默认）/ 'all' = 全部
  let todoViewMode = 'day';
  // 全部模式下按日期分组的扁平化列表（每项带 groupKey 用于渲染分组标题）
  let allTodosGrouped = [];
  ...
  function setTodoViewMode(mode) {
    if (mode !== 'day' && mode !== 'all') return;
    todoViewMode = mode;
    const screen = document.getElementById('todo-list-screen');
    if (screen) screen.classList.toggle('in-all-mode', mode === 'all');
    updateTodoViewToggle();
    updateTodoDateDisplay();
    renderTodoList();
  }
  ...
  // 更新顶部 '全部/单天' 切换按钮的视觉态
  function updateTodoViewToggle() {
    const toggleBtn = document.getElementById('todo-view-all-btn');
    if (!toggleBtn) return;
    if (todoViewMode === 'all') {
      toggleBtn.classList.add('active');
      toggleBtn.textContent = '单天';
    } else {
  ...
  function changeTodoDate(days) {
    // 全部模式下 prev/next 不改变日期（按钮会被 CSS 禁用，但 JS 也兜底）
    if (todoViewMode !== 'day') return;
  ...
  async function renderTodoList() {
    ...
    if (!chat) return;
    if (todoViewMode === 'all') {
      await renderAllTodos(chat);
    } else {
      await renderDayTodos(chat);
    }
  }

  // 单天视图（原逻辑，行为不变）
  async function renderDayTodos(chat) {
    const container = document.getElementById('todo-list-container');
  ...
  // 全部视图：按日期分组（今天 > 未来 > 过去，每组内 pending 优先，按 time 升序）
  async function renderAllTodos(chat) {
    const container = document.getElementById('todo-list-container');
    const todos = (chat.todoList || []).slice();
    if (todos.length === 0) {
      container.innerHTML = `<div class="todo-empty-state">📋 全部待办<br>暂无任何待办事项</div>`;
      return;
    }
    const todayStr = getTodoDateString(new Date());
    // 按日期分组
    const groups = new Map();
    todos.forEach(t => {
      const key = t.date || '未排期';
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(t);
    });
    // 排序：今天 > 未来日期（升序） > 过去日期（降序） > 未排期
    ...
    // 展平成渲染序列：每组 [分组标题, ...todo, 分组间距]
    const flat = [];
    sortedKeys.forEach(key => {
      const groupTodos = groups.get(key).slice().sort((a, b) => {
        if (a.status === b.status) return (a.time || '00:00').localeCompare(b.time || '00:00');
        return a.status === 'completed' ? 1 : -1;
      });
      const label = key === todayStr ? '今天' : (key === '未排期' ? '未排期' : key);
      flat.push({ kind: 'header', key, label, count: groupTodos.length, pendingCount: groupTodos.filter(t => t.status !== 'completed').length });
      groupTodos.forEach(t => flat.push({ kind: 'todo', todo: t }));
    });
    // 顶部统计
    const totalCount = todos.length;
    const totalPending = todos.filter(t => t.status !== 'completed').length;
    container.innerHTML = `<div class="todo-all-summary">全部待办 · 共 ${totalCount} 项 · 未完成 ${totalPending}</div>`;
    allTodosGrouped = flat;
    todoRenderCount = 0;
    loadMoreAllTodos();
  }

  function loadMoreAllTodos() {
    if (isLoadingMoreTodos) return;
    const container = document.getElementById('todo-list-container');
    if (!container) return;
    if (todoRenderCount >= allTodosGrouped.length) return;
    isLoadingMoreTodos = true;
    const BATCH_SIZE = 30;
    const nextSliceEnd = todoRenderCount + BATCH_SIZE;
    const itemsToRender = allTodosGrouped.slice(todoRenderCount, nextSliceEnd);
    const fragment = document.createDocumentFragment();
    let lastHeaderEl = null;
    itemsToRender.forEach(entry => {
      if (entry.kind === 'header') {
        const headerEl = document.createElement('div');
        headerEl.className = 'todo-group-header';
        const overdueCls = (entry.key < getTodoDateString(new Date()) && entry.pendingCount > 0) ? ' overdue' : '';
        headerEl.innerHTML = `<span class="todo-group-label${overdueCls}">${escapeHTML(entry.label)}</span><span class="todo-group-count">${entry.pendingCount}/${entry.count}</span>`;
        fragment.appendChild(headerEl);
        lastHeaderEl = headerEl;
      } else {
        fragment.appendChild(buildTodoItemEl(entry.todo));
      }
    });
  ...
  // 抽出 todo 卡片 DOM 构造，复用于 day/all 两种视图
  function buildTodoItemEl(todo) {
    const item = document.createElement('div');
    ...
    // AI 添加的卡片右上角加个徽章，方便一眼区分来源
    const aiBadge = isUser ? '' : '<span class="todo-ai-badge" title="这条待办是 AI 根据对话自动添加的">🤖 AI 添加</span>';
    item.innerHTML = `
            <div class="todo-checkbox"></div>
            <div class="todo-info">
                <div class="todo-content">${escapeHTML(todo.content)}${aiBadge}</div>
                <div class="todo-meta">
                    <span class="todo-tag" style="--tag-color: ${tagColor};">${todo.type || '日常'}</span>
                    ${todo.time ? `<span class="todo-time">⏰ ${todo.time}</span>` : ''}
                    ${todo.date ? `<span class="todo-date-tag">📅 ${todo.date}</span>` : ''}
                </div>
            </div>
  ...
  function loadMoreTodos() {
    ...
    const fragment = document.createDocumentFragment();
    itemsToRender.forEach(todo => fragment.appendChild(buildTodoItemEl(todo)));
    container.appendChild(fragment);
    todoRenderCount += itemsToRender.length;
    isLoadingMoreTodos = false;
  }
  ...
  // ========== 批量清理：清空 AI 添加的 / 过期的 / 已完成的 ==========
  // 过期判断：date 严格小于今天（昨天、前天、半年前都算过期）
  function isTodoExpired(dateStr) {
  ...
  // 打开批量清理 modal（先统计当前各项数量展示给用户）
  function openTodoManageModal() {
    const chat = state.chats[state.activeChatId];
    if (!chat) return;
    const todos = chat.todoList || [];
    const aiCount = todos.filter(t => t.creator === 'char').length;
    const overdueCount = todos.filter(t => t.status !== 'completed' && isTodoExpired(t.date)).length;
    const completedCount = todos.filter(t => t.status === 'completed').length;
    const modal = document.getElementById('todo-manage-modal');
    if (!modal) return;
    const aiOpt = document.getElementById('todo-manage-ai-opt');
    const overdueOpt = document.getElementById('todo-manage-overdue-opt');
    const completedOpt = document.getElementById('todo-manage-completed-opt');
    if (aiOpt) aiOpt.querySelector('.todo-manage-count').textContent = `${aiCount} 项`;
    if (overdueOpt) overdueOpt.querySelector('.todo-manage-count').textContent = `${overdueCount} 项`;
    if (completedOpt) completedOpt.querySelector('.todo-manage-count').textContent = `${completedCount} 项`;
    // 没有数据时禁用对应行
    [aiOpt, overdueOpt, completedOpt].forEach(el => {
      if (!el) return;
      const count = parseInt(el.querySelector('.todo-manage-count').textContent, 10) || 0;
      if (count === 0) el.classList.add('disabled');
  ...
  // 执行批量清理（kind: 'ai' | 'overdue' | 'completed'）
  async function executeTodoManage(kind) {
    const chat = state.chats[state.activeChatId];
    if (!chat || !chat.todoList) return;
    const before = chat.todoList.length;
    let removed = 0;
    if (kind === 'ai') {
      chat.todoList = chat.todoList.filter(t => t.creator !== 'char');
      removed = before - chat.todoList.length;
    } else if (kind === 'overdue') {
      chat.todoList = chat.todoList.filter(t => {
        const isOverdue = t.status !== 'completed' && isTodoExpired(t.date);
        return !isOverdue;
      });
      removed = before - chat.todoList.length;
    } else if (kind === 'completed') {
      chat.todoList = chat.todoList.filter(t => t.status !== 'completed');
      removed = before - chat.todoList.length;
    }
    if (removed > 0) {
      await db.chats.put(chat);
    ...
    closeTodoManageModal();
    if (removed > 0) {
      showCustomAlert("清理完成", `已删除 ${removed} 项待办。`);
    } else {
      showCustomAlert("没有可清理的", "未找到符合条件的事项。");
    }
  }
  ...
  window.updateTodoViewToggle = updateTodoViewToggle;
  window.openTodoManageModal = openTodoManageModal;
  window.closeTodoManageModal = closeTodoManageModal;
  window.executeTodoManage = executeTodoManage;
