...
// ============================================================
  ...
  function handleCharacterFileImport() {
    return new Promise(resolve => {
      ...
      input.type = 'file';
      input.accept = '.txt,.docx,.zip,.json,.png'; // 支持 TXT、DOCX、ZIP、JSON 和 PNG
      ...
      input.onchange = async e => {
        ...
        for (let i = 0; i < normalFiles.length; i++) {
          ...
          try {
            // 直接处理 JSON/PNG 角色卡文件
            if (file.name.toLowerCase().endsWith('.json') || file.name.toLowerCase().endsWith('.png')) {
              await processCharacterCardFile(file);
              importedCount++;
              continue;
            }
  ...
  async function processCharacterFile(file) {
    ...
    throw new Error("不支持的文件格式，仅支持 .txt、.docx、.json、.png 或 ZIP");
  }

  async function processCharacterCardFile(file) {
    const fileName = file.name.toLowerCase();
    ...
    if (fileName.endsWith('.json')) {
      ...
      if (parsed.type === 'EPhoneCharacterExport') {
        await importEPhoneCharacter(parsed, null, true);
        return;
      }

      await createChatFromCardData(parsed, null, true);
      return;
    }
    ...
    if (fileName.endsWith('.png')) {
      ...
      if (formats.ephone && formats.ephone.type === 'EPhoneCharacterExport') {
        ...
        await importEPhoneCharacter(formats.ephone, avatarBase64, true);
        return;
      }
      ...
      if (formats.chara) {
        ...
        await createChatFromCardData(formats.chara, avatarBase64, true);
        return;
      }
    ...
    throw new Error('处理此文件类型的角色卡导入失败。');
  }
  ...
  async function handleZipFileImport(zipFile) {
    try {
      ...
      for (const [filename, zipEntry] of Object.entries(zip.files)) {
        ...
        if (lowerName.endsWith('.txt') || lowerName.endsWith('.docx') || lowerName.endsWith('.json') || lowerName.endsWith('.png')) {
          fileEntries.push({
            filename: filename,
            zipEntry: zipEntry,
            type: lowerName.endsWith('.txt') ? 'txt' : lowerName.endsWith('.docx') ? 'docx' : lowerName.endsWith('.json') ? 'json' : 'png'
          });
        }
      }

      if (fileEntries.length === 0) {
        await showCustomAlert('ZIP文件为空', `ZIP文件"${zipFile.name}"中没有找到 .txt、.docx、.json 或 .png 文件。`);
        return;
      }
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
          ...
          if (entry.type === 'txt') {
            ...
            if (formats.chara) {
              textContent = JSON.stringify(formats.chara, null, 2);
            } else {
              await showCustomAlert('文件类型不支持', `PNG文件"${entry.filename}"中未找到有效的角色卡数据，已跳过。`);
              skippedCount++;
              continue;
            }
          }
  ...
  async function createChatFromCardData(cardData, avatarBase64 = null, silent = false) {
