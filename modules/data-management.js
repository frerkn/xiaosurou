...
async function cleanupRedundantData() {
  ...
  async function resetFontByScope() {
    ...
    const scopeLabels = {
      ...
      myphone: 'Myphone', worldBook: '世界书', hotNews: '热点日报',
    ...
    return new Promise(resolve => {
      ...
      confirmBtn.onclick = async () => {
        ...
        if (!anyActive) {
          ...
          state.globalSettings.fontScope = { all: true, homeScreen: true, qq: true, cphone: true, myphone: true, worldBook: true, hotNews: true, alipay: true, settings: true };
          dynamicFontStyle.innerHTML = '';
  ...
  async function renderClearChatImagesList() {
    ...
    for (const chat of allChats) {
      ...
      const imageStats = await countUserImagesForChat(chat.id);
  ...
  const CLEAR_CHAT_IMAGE_AI_TYPES = new Set(['ai_image', 'naiimag', 'googleimag', 'openaiimag']);

  function isClearChatAiImageMessage(msg) {
    return !!(msg && typeof msg === 'object' && CLEAR_CHAT_IMAGE_AI_TYPES.has(msg.type));
  }

  function isClearChatLocalImageUrl(value) {
    return typeof value === 'string' && value.startsWith('data:image');
  }

  function isClearChatImageFileValue(value) {
  ...
  function isClearChatUserImageMessage(msg) {
    if (!msg || typeof msg !== 'object') return false;
    if (CLEAR_CHAT_IMAGE_AI_TYPES.has(msg.type)) return false;
    if (msg.role && msg.role !== 'user') return false;

    if (msg.images && Array.isArray(msg.images) && msg.images.some(isClearChatLocalImageUrl)) return true;

    if (Array.isArray(msg.content)) {
      return msg.content.some((item) =>
        item?.type === 'image_url' &&
        item.image_url?.url &&
        isClearChatLocalImageUrl(item.image_url.url)
      );
    }

    if (isClearChatLocalImageUrl(msg.content)) return true;
    if (isClearChatLocalImageUrl(msg.imageSrc)) return true;
    if (isClearChatImageFileValue(msg.file)) return true;

    return false;
  }

  function clearUserImagesFromMessage(msg, stats) {
    if (!isClearChatUserImageMessage(msg)) return false;

    let changed = false;

    // 清空 msg.images 格式（旧格式），仅移除本地用户图片
    if (msg.images && Array.isArray(msg.images)) {
      const retainedImages = [];
      for (const img of msg.images) {
        if (isClearChatLocalImageUrl(img)) {
          stats.imagesCleared++;
      ...
      if (changed) msg.images = retainedImages;
    }

    // 清空 msg.content 中的图片（新格式），保留 image_url 项，让现有渲染逻辑显示原可爱占位图
    if (Array.isArray(msg.content)) {
      let contentChanged = false;
      msg.content = msg.content.map((item) => {
        if (item?.type === 'image_url' && item.image_url?.url && isClearChatLocalImageUrl(item.image_url.url)) {
          stats.imagesCleared++;
          stats.sizeFreed += item.image_url.url.length;
          changed = true;
          contentChanged = true;
          return {
            ...item,
    ...
    } else if (isClearChatLocalImageUrl(msg.content)) {
      stats.imagesCleared++;
      stats.sizeFreed += msg.content.length;
      msg.content = '';
      changed = true;
    }

    if (isClearChatLocalImageUrl(msg.imageSrc)) {
      stats.imagesCleared++;
      stats.sizeFreed += msg.imageSrc.length;
      delete msg.imageSrc;
      changed = true;
    }

    if (isClearChatImageFileValue(msg.file)) {
      stats.imagesCleared++;
      delete msg.file;
      changed = true;
    }

    // 注意：保留 imageProcessed / imageDescription / description / 文本 content 项，不删除识图描述。
    return changed;
  }

  async function countUserImagesForChat(chatId) {
    const chat = await db.chats.get(chatId);
    const stats = { imagesCleared: 0, aiImagesDeleted: 0, sizeFreed: 0 };

    const sourceMessages = db.messages
      ? await db.messages.where('chatId').equals(chatId).toArray()
      : (chat?.history && Array.isArray(chat.history) ? chat.history : []);

    for (const msg of sourceMessages) {
      if (isClearChatAiImageMessage(msg)) {
        stats.aiImagesDeleted++;
        continue;
      }
      clearUserImagesFromMessage(JSON.parse(JSON.stringify(msg)), stats);
    }

    return stats;
  }

  async function clearUserImagesForChat(chatId, stats) {
    const chat = await db.chats.get(chatId);
    if (!chat) return;

    let chatProcessed = false;
    const hasMessagesTable = !!db.messages;

    const aiMessageIdsToDelete = [];
    ...
    if (hasMessagesTable) {
      ...
      for (const msg of allMessages) {
        if (isClearChatAiImageMessage(msg)) {
          if (msg.id) aiMessageIdsToDelete.push(msg.id);
          continue;
        }
        if (clearUserImagesFromMessage(msg, stats)) {
      ...
      if (aiMessageIdsToDelete.length > 0) {
        await db.messages.bulkDelete(aiMessageIdsToDelete);
        stats.aiImagesDeleted = (stats.aiImagesDeleted || 0) + aiMessageIdsToDelete.length;
        chatProcessed = true;
      }
    }

    if (chat.history && Array.isArray(chat.history)) {
      const historyStats = hasMessagesTable ? { imagesCleared: 0, sizeFreed: 0 } : stats;
      const originalHistoryLength = chat.history.length;
      chat.history = chat.history.filter(msg => !isClearChatAiImageMessage(msg));
      if (!hasMessagesTable && chat.history.length < originalHistoryLength) {
        stats.aiImagesDeleted = (stats.aiImagesDeleted || 0) + (originalHistoryLength - chat.history.length);
        chatProcessed = true;
      }
      for (const msg of chat.history) {
        if (clearUserImagesFromMessage(msg, historyStats)) {
          chatProcessed = true;
        }
      }
      await db.chats.put(chat);
    ...
    const syncOnlyStats = { imagesCleared: 0, sizeFreed: 0 };

    const stateChat = state.chats?.[chatId];
    if (stateChat?.history && Array.isArray(stateChat.history)) {
      stateChat.history = stateChat.history.filter(msg => !isClearChatAiImageMessage(msg));
      for (const msg of stateChat.history) {
        clearUserImagesFromMessage(msg, syncOnlyStats);
      }
    }

    const windowMessages = window.messageStore?.getWindowMessages
      ? window.messageStore.getWindowMessages(chatId)
      : null;
    if (Array.isArray(windowMessages)) {
      const retainedWindowMessages = windowMessages.filter(msg => !isClearChatAiImageMessage(msg));
      if (retainedWindowMessages.length !== windowMessages.length && window.messageStore?.setWindow) {
        window.messageStore.setWindow(chatId, retainedWindowMessages, {
          hasMore: state.messageWindows?.[chatId]?.hasMore !== false
        });
      }
      for (const msg of retainedWindowMessages) {
        clearUserImagesFromMessage(msg, syncOnlyStats);
      }
    }

    if (state.activeChatId === chatId && Array.isArray(state.currentMessages)) {
      state.currentMessages = state.currentMessages.filter(msg => !isClearChatAiImageMessage(msg));
    }

    if (chatProcessed && window.messageStore?.refreshChatMeta) {
      await window.messageStore.refreshChatMeta(chatId);
    }

    if (chatProcessed) stats.chatsProcessed++;
  }

  window.clearUserImagesForChat = clearUserImagesForChat;
  window.clearUserImagesFromMessage = clearUserImagesFromMessage;
  window.countUserImagesForChat = countUserImagesForChat;
  ...
  async function handleConfirmClearChatImages() {
    ...
    try {
      ...
      for (const chatId of selectedCharsForImageClear) {
        await clearUserImagesForChat(chatId, stats);
      }
      ...
      const shouldRefresh = await showCustomConfirm(
        ...
        清空了 ${stats.imagesCleared} 张用户图片<br>
        删除了 ${stats.aiImagesDeleted || 0} 条 AI 图片消息<br>
  ...
  // 在控制台运行此函数来检查指定聊天的图片数据
  window.debugCheckImagesInChat = async function(chatId) {
    const chat = await db.chats.get(chatId);
    if (!chat) {
      console.log('聊天不存在:', chatId);
      return;
    }
    
    console.log(`检查聊天 "${chat.name}" 中的图片数据:`);
    let imageCount = 0;
    ...
    if (chat.history && Array.isArray(chat.history)) {
      chat.history.forEach((msg, index) => {
        ...
        if (msg.images && Array.isArray(msg.images)) {
          msg.images.forEach(img => {
            if (typeof img === 'string' && img.startsWith('data:image')) {
              console.log(`消息 ${index}: 包含base64图片数据 (images数组), 大小: ${(img.length / 1024).toFixed(1)}KB`);
              hasImage = true;
              imageCount++;
            }
          });
          ...
          msg.content.forEach(item => {
            if (item.type === 'image_url' && item.image_url && item.image_url.url) {
              ...
              if (typeof url === 'string' && url.startsWith('data:image')) {
                console.log(`消息 ${index}: 包含base64图片数据 (content数组), 大小: ${(url.length / 1024).toFixed(1)}KB`);
                hasImage = true;
                imageCount++;
              }
            }
        ...
        // 检查直接base64字符串
          ...
          console.log(`消息 ${index}: 包含base64图片数据 (直接字符串), 大小: ${(msg.content.length / 1024).toFixed(1)}KB`);
          hasImage = true;
          imageCount++;
        }
        
        // 检查识图优化状态
        if (msg.imageProcessed) {
          console.log(`消息 ${index}: 已处理过识图优化, 描述: "${msg.imageDescription || '无'}"`);
        }
        
        if (!hasImage && (msg.imageProcessed || msg.imageDescription)) {
          console.log(`消息 ${index}: 无图片数据但有优化标记`);
        }
      });
    }
    
    console.log(`总共发现 ${imageCount} 个图片数据`);
    return imageCount;
  };
...
// ========== 钱包交易处理（从 script.js 补充拆分，原第 60109~60165 行） ==========
  ...
  async function convertLongTermMemoryToStructured(chatId) {
    ...
    const secondaryConfig = typeof resolveApiSlotConfig === 'function'
      ? await resolveApiSlotConfig('secondary')
      : {
        proxyUrl: state.apiConfig.secondaryProxyUrl,
        apiKey: state.apiConfig.secondaryApiKey,
        model: state.apiConfig.secondaryModel
      };
    const mainConfig = typeof resolveApiSlotConfig === 'function'
      ? await resolveApiSlotConfig('main')
      : state.apiConfig;
    const { proxyUrl, apiKey, model } = (secondaryConfig?.proxyUrl && secondaryConfig?.apiKey && secondaryConfig?.model)
      ? secondaryConfig
      : mainConfig;
