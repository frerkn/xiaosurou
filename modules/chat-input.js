...
// ============================================================
    ...
    if (paymentMethod === 'balance') {
      ...
      if (cardIndex > -1) {
        ...
        if (providerChat) {
          ...
          const notifyMessage = { role: 'system', content: notifyContent, timestamp: Date.now(), isHidden: true };
          if (window.messageStore) {
            await window.messageStore.addMessageToChat(providerChat, notifyMessage);
          } else {
            providerChat.history.push(notifyMessage);
  ...
  async function sendWaimaiOrderForAI() {
    ...
    if (window.messageStore) {
      await window.messageStore.addMessageToChat(chat, visibleMessage);
    } else {
    ...
    if (window.messageStore) {
      await window.messageStore.addMessageToChat(chat, hiddenMessage);
    } else {
  ...
  async function startReplyToMessage() {
    if (!activeMessageTimestamp) return;

    const chat = state.chats[state.activeChatId];
    const message = window.findChatMessageByTimestamp
      ? await window.findChatMessageByTimestamp(chat, activeMessageTimestamp, { includeHidden: false })
      : chat.history.find(m => Number(m.timestamp) === Number(activeMessageTimestamp) && !m.isHidden);
    if (!message) return;
...
// ========== 转账响应处理（从 script.js 补充拆分，原第 26814~26917 行） ==========
  ...
  async function handleUserTransferResponse(choice) {
    ...
    const originalMessage = window.messageStore
      ? await window.messageStore.getMessageByTimestamp(chat.id, timestamp)
    ...
    if (window.messageStore) {
      await window.messageStore.updateMessage(chat.id, timestamp, { status: choice });
    } else {
    ...
    if (choice === 'declined') {
      ...
      if (window.messageStore) {
        await window.messageStore.addMessageToChat(chat, refundMessage);
      } else {
      ...
      if (transferAmount > 0) {
        ...
        if (success) {
          ...
          if (window.messageStore) {
            await window.messageStore.addMessageToChat(chat, receivedMessage);
          } else {
